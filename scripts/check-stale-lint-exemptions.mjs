 ...
 *
 * BU-5 / BU-6 (docs/20260819-built-unlanded-and-unwired-register-v1.00W.md):
 ...
 * gitignored build artifact that is genuinely present on disk. The second
 * half matters: `eslint-config-next`'s own recommended config ignores
 * `next-env.d.ts`, a file Next.js regenerates on every build and most teams
 * (this estate included) never commit. That ignore entry is current,
 * correct, third-party-authored config, not a bsuite-authored exemption that
 * outlived its subject — `git ls-files` alone would misclassify it as
 * missing. A path that is neither tracked nor present-and-ignored is the
 * real defect: gone from source, still named in config.
 */
function pathStillExists(repoDir, relPath) {
  if (isTracked(repoDir, relPath)) return true
  const onDisk = fs.existsSync(path.join(repoDir, relPath))
  return onDisk && isGitIgnored(repoDir, relPath)
}
