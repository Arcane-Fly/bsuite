...
function darkStrategy(appDir, cssRel) {
  ...
  if (configMatch) {
    const cfgPath = resolve(dirname(cssPath), configMatch[1])
    if (existsSync(cfgPath) && DARK_MODE_CLASS.test(readFileSync(cfgPath, 'utf8'))) {
      return `@config -> ${configMatch[1]} with darkMode:'class'`
    }
  }
  return null
}
...
if (process.argv.includes('--self-test')) {
  ...
  console.log(`check-dark-variant-strategy --self-test: OK (${cases.length} cases)`)
  process.exit(0)
}
