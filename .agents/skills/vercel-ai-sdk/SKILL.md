...
| Skill | Use When |
|-------|----------|
| [`supabase-auth-comprehensive`](.agents/skills/supabase-auth-comprehensive/SKILL.md) | MCP authentication for AI agents (Claude Code, Gemini CLI) — OAuth 2.1 resource server pattern (RFC 9728), token validation, custom access token hooks |
| [`vercel-next-best-practices`](.agents/skills/vercel-next-best-practices/SKILL.md) | Next.js patterns for AI app routes, RSC boundaries, data patterns |
| [`vercel-react-best-practices`](.agents/skills/vercel-react-best-practices/SKILL.md) | React performance for AI-powered UIs — streaming, re-render optimization |
