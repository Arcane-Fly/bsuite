...
PRIVATE_KEY = "<RSA PRIVATE KEY PEM block>"
```
...
| Skill | Use When |
|-------|----------|
| [`supabase-auth-comprehensive`](.agents/skills/supabase-auth-comprehensive/SKILL.md) | Supabase auth security — JWT signing keys (RS256/ES256), PKCE flows, password policies, rate limits, CAPTCHA, session hardening, `getSession()` vs `getClaims()` |
| [`supabase-oauth-server`](.agents/skills/supabase-oauth-server/SKILL.md) | OAuth 2.1 security — redirect URI validation, token validation tiers, cross-subdomain cookie security |
| [`auth-setup`](.agents/skills/auth-setup/SKILL.md) | Multi-project auth audit — surface inventory, hardening checklist, red-team verification |
| [`supabase-postgres-best-practices`](.agents/skills/supabase-postgres-best-practices/SKILL.md) | Database security — RLS policies, SQL injection prevention, connection security |
