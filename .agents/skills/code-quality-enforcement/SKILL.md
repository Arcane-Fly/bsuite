...
## DRY at the entity layer

General DRY (no duplicated functions / utilities / constants) is table
stakes. For BSuite and general-SaaS projects, entity-level DRY is an
additional layer — captured in the **`dry-one-shot-architecture`**
skill. Cross-reference it when reviewing anything that touches entity
CRUD, forms, or cross-app data flow. Specific entity-level violations:

- Duplicated entity forms across apps
- Local state that mirrors a Supabase table
- `CREATE TABLE IF NOT EXISTS` for entities owned by another app
- Free-text columns `*_name` / `*_email` / `*_phone` / `*_code` /
  `*_company` where an FK to a canonical table already exists
- Frontend-only permission checks (must have RLS)
- `workspace:*` / `file:../packages/*` in deployable `package.json`

Canonical spec: `docs/20260227-dry-one-shot-architecture-v1.01A.md`.
