...
describe('edit-mode editor chrome is a measured in-flow strip', () => {
  ...
  /**
   * The narrow-card contract. jsdom has no layout engine, so it cannot be asked
   * whether the hide button LEAVES the card box — that measurement lives in
   * scripts/editor-chrome-geometry.mjs, which reproduces the defect at 1440 and
   * 768 by removing exactly these two classes at run time. What jsdom CAN carry
   * is that the classes are still declared and the full name is still reachable,
   * so the fix cannot be deleted silently.
   */
  it('the strip declares min-w-0 so it can shrink inside its column', () => {
    const { container } = mount();
    openEditor();
    const strip = editorChrome(container)!;
    expect(
      strip.classList.contains('min-w-0'),
      'the strip must be allowed to shrink below its content width',
    ).toBe(true);
  });

  it('the label declares truncate and keeps the full name in title, so a long name clips instead of pushing hide out', () => {
    const { container } = mount();
    openEditor();
    const strip = editorChrome(container)!;
    const label = [...strip.querySelectorAll('span')].find((el) =>
    ...
    expect(
      ...
      'without truncate the label cannot shrink below min-content and hide is pushed outside the card',
    ).toBe(true);
    expect(label.classList.contains('min-w-0')).toBe(true);
    expect(
      label.getAttribute('title'),
      'clipping must not lose the name — title carries it in full',
    ).toBe('Candidates');
    // Truncation is the ONLY concession. Wrapping to a second line would change
    // the measured strip height that autoHeight feeds back to the grid.
    expect([...label.classList].some((c) => c.startsWith('line-clamp-'))).toBe(false);
  });

  it('the hide control stays shrink-0 so truncation is taken from the label, never the button', () => {
    const { container } = mount();
    openEditor();
    const hide = editorChrome(container)!.querySelector<HTMLButtonElement>(
      '[aria-label="Hide Candidates"]',
    )!;
    expect(hide.classList.contains('shrink-0')).toBe(true);
    expect(hide.classList.contains('h-6')).toBe(true);
    expect(hide.classList.contains('w-6')).toBe(true);
  });
