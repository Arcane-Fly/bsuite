...
export function JobDistributeView() {
  const params = useParams()
  const router = useRouter()
  const { tenantId } = useTenantId()
  const { createDistribution, markPosted, markExpired, deleteDistribution } = useJobDistributionStore()
  ...
  const { data: jobDistributions = [], isLoading: distLoading, error: distError } = useQuery<JobDistribution[]>({
    queryKey: ['job-distributions', tenantId, jobId],
    queryFn: async () => {
      if (!tenantId) return []
      const supabase = createClient()
      const { data, error } = await supabase
  ...
  const loading = distLoading
  const error = distError ? String(distError) : null
  ...
  async function handleMarkPosted(id: string) {
    ...
    setActiveMenu(null)
    void queryClient.invalidateQueries({ queryKey: ['job-distributions', tenantId, jobId] })
  }
