...
export function EditJobView() {
  ...
  // useTenantId is available for future use (e.g. RLS checks)
  useTenantId()
  ...
  const { data: job, isLoading: loading } = useQuery<Job | null>({
    queryKey: ['job', jobId],
    queryFn: async () => {
  ...
  useEffect(() => {
    if (job) {
      setForm({
        ...
        closing_date: job.closing_date ? job.closing_date.slice(0, 10) : '',
        award_code: job.award_code ?? '',
      })
    }
  }, [job])
