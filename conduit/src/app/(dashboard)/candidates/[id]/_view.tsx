...
export function CandidateDetailView() {
  ...
  const { data: candidate, isLoading: loading } = useQuery<Candidate | null>({
  ...
  // Initialise edit form when candidate loads
  const currentEditForm = Object.keys(editForm).length === 0 && candidate ? candidate : editForm

  async function handleSave() {
    if (!candidate) return
    const ok = await updateCandidate(candidate.id, currentEditForm)
    if (ok) {
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="rounded-lg border p-4 space-y-3">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <InfoRow icon={Mail} label="Email" value={candidate.email} editing={editing} field="email" form={currentEditForm} setForm={setEditForm} />
              <InfoRow icon={Phone} label="Phone" value={candidate.phone} editing={editing} field="phone" form={currentEditForm} setForm={setEditForm} />
          ...
          <div className="rounded-lg border p-4 space-y-3">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <InfoRow icon={Clock} label="Availability" value={candidate.availability} editing={editing} field="availability" form={currentEditForm} setForm={setEditForm} />
