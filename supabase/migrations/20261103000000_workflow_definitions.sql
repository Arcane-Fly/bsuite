...
--
--   * on disk — 0 files at 20261103000000 across all 10 migration directories
--     in the estate, including crm7's archive/ and baseline/ and the root
--     baseline/
--   * in the ledger — `SELECT count(*) ... WHERE version = '20261103000000'`
--     returned 0 against the shared project (tuybltdrdefjblnplpqo); global max
--     applied is 20261101000000 across 798 rows
--
-- RENUMBERED FROM 20261102000000, and the reason is worth keeping. That version
-- was genuinely free when this file was written — 0 on disk, 0 in the ledger,
-- both checked. It stopped being free WHILE THIS WAS IN REVIEW: another lane
-- merged 20261102000000_training_contracts_training_plan_fk.sql (bsuite#2896)
-- into development, in all three of the same scopes. crm7's own
-- `no-duplicate-migration-version` lint caught it on the first CI run.
--
-- The lesson is that a version census is a SNAPSHOT, not a reservation. Nothing
-- holds a version between authoring and merge, so the check has to be re-run
-- against the branch that will actually be tested — not the tree the work
-- started from. This file's first census was taken against a gitlink six
-- commits behind development, and was accurate about a tree CI would never see.
--
