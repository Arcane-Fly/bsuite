...
--
-- IT RUNS AS THE CALLER, and that is now written in the function itself rather
-- than left to plpgsql's default. The caller's own RLS governs every row:
-- tenant_settings_update_admin and tenant_settings_insert_admin both scope to
...
create or replace function public.set_tenant_oncosts(
  ...
  language plpgsql
  -- Stated, not implied. plpgsql defaults to the caller's privileges, but a
  -- default a reader has to know is a default a reader can get wrong.
  security invoker
...
comment on function public.set_tenant_oncosts(uuid, numeric, numeric, text) is
  'bsuite estate register W-8. Runs as the CALLER — the caller''s own RLS on '
