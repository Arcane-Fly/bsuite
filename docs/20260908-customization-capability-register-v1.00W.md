...
**Acceptance corrections remain open:** [crm7#2649](https://github.com/GaryOcean428/crm7/pull/2649) corrects grid interception of Enter/Space/Tab on related-record actions and responsive client headers; signed head `d0c85511d`, 24 focused tests and independent review passed. Browser coverage includes 375/390/768/1440 in both themes, with action bounds, keyboard focus and screenshot artifacts; current hosted execution and exact deployed retest remain pending. A live catalogue read also found that `vacancies.client_id` and `vacancies.host_employer_id` have real foreign keys, while the vacancies catalogue entity and joins are absent: C03/[bsuite#3207](https://github.com/GaryOcean428/bsuite/issues/3207) requires curated metadata repair, preserving the fail-closed picker gate. Eighteen dark-theme resting accent-shadow failures remain under [crm7#2506](https://github.com/GaryOcean428/crm7/issues/2506), dependent on the existing shared-package/consumer release lane. Full D8 is not approved.
