...
| 3.5 | **`quality.yml` deletion** — no trigger so it never runs, duplicates `ci.yml`, referenced by no branch protection. Evidence is good; deleting pre-existing work is the operator's call. Branch `fix/ci-guards-deferred-items-20260811` holds it. |
