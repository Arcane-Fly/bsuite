...
| 21 | docs/00-roadmap/20260812-estate-remaining-work-register-v1.00F.md | SUPERSEDED | Opened the source: its banner explicitly says `SUPERSEDED — FROZEN 2026-08-26` and names `docs/20260817-estate-remaining-work-register-v3.00W.md` as the successor. This establishes document supersession, not the successor's current issue/source/deployment correctness. |
...
| 33 | docs/20260814-notes-backlog-verification-register-v1.00F.md | SUPERSEDED | Opened the source: its banner says `FROZEN 2026-08-29 — SUPERSEDED, AND THE SUPERSESSION IS MEASURED` and names `docs/20260825-operator-notes-register-d1-d103-v1.00W.md` as the successor/current register. This establishes document supersession, not the successor's current issue/source/deployment correctness. |
...
| 39 | Replayable negative evidence from `/home/braden/Desktop/Dev/bsuite`: `grep -n -F "InvoiceLineItemBuilder" crm7/eslint.config.js`, `grep -n -F "report-form-dialog" crm7/eslint.config.js`, and `grep -n -F "user-management" throughput/eslint.config.js` each returned no stdout and exit **1**; the equivalent scoped `git -C crm7 grep ...` / `git -C throughput grep ...` searches against `*eslint.config.*` also returned no matches/exit **1**. Positive control: `grep -n -E "InvoiceLineItemBuilder|report-form-dialog|user-management" docs/20260817-built-unlanded-and-unwired-register-v1.00W.md` returned the historical anchors `:186-187`, `:206`, `:208`, `:211`, and `:545`. | This proves only that the named current lint/config matches are absent. Reconcile each item with its historical source/config context, then verify import, route, saved-state/read-back, authenticated browser reachability, deployment identity, and live data/RLS behaviour. |
...
| 74 | JSON parsing succeeded (`generated=2026-09-02`). Structural replay found **5** RLS ownership; security **2 attributable + 3 unattributable**; journey gaps **4**, config-without-editor **3**, bulk **1**, seam **1**, walked **4**, not-yet-walked **4**; operator findings **142**; themes **11**; and `issues_filed` has **4** top-level keys and **0 failures**. The stored aggregate says `repeat_offences: 26`, while an independent count of records with `repeat_offence === true` is **28**; the internal mismatch must be reconciled before trusting the aggregate. | Counts substantiate stored shape and declared totals, not current issue/PR status, deployment, or live RLS. Reconcile references with the tracker and authenticated behaviour. |
