...
canvas):
  ...
  node -e "const fs=require('fs'); for (const [n,p] of Object.entries({AppShell:'packages/ui/src/app-shell.tsx',Button:'packages/ui/src/button.tsx',DataGrid:'packages/data-grid/src/DataGrid.tsx',DraggableCardPage:'packages/page-builder/src/canvasCardLayout.tsx',RelationshipCanvas:'business-suite-unified/src/components/feature-builder/RelationshipCanvas.tsx'})) console.log(n+': '+(fs.existsSync(p)?p:'MISSING'))"
  AppShell: packages/ui/src/app-shell.tsx
  Button: packages/ui/src/button.tsx
  DataGrid: packages/data-grid/src/DataGrid.tsx
  DraggableCardPage: packages/page-builder/src/canvasCardLayout.tsx
...
`scripts/generate-component-registry.mjs`):
  ...
  node -e "const r=require('./docs/00-roadmap/bsuite-component-registry.json'); for (const n of ['AppShell','Button','DataGrid','DraggableCardPage']) { const c=r.components.find(x=>x.name===n); const apps=Object.keys(c.consumers); const label=n==='Button'?'0 direct shared-package app consumers':c.consumerFiles+' consumers / '+c.consumingApps+(c.consumingApps===1?' app':' apps')+(apps.length?' ('+apps.join(', ')+')':''); console.log(n+': '+label); } console.log('RelationshipCanvas: local / absent from shared registry')"
  ...
  RelationshipCanvas: local / absent from shared registry
