...
| 2.2 RLS initplan rewrite | 25 direct-auth policies initially; Visual Feature Builder + role_capabilities slices done; 11 direct-auth policies remain when including `auth.role()` wrappers | 🟡 IN PROGRESS | CRM7 PR [#963](https://github.com/GaryOcean428/crm7/pull/963) rewrites the Visual Feature Builder policy group to `(SELECT auth.uid())` initplan wrappers and adds `09_feature_builder_rls_initplan.sql` pgTAP coverage. CRM7 PR [#964](https://github.com/GaryOcean428/crm7/pull/964) rewrites `role_capabilities` and adds `09_role_capabilities_rls_initplan.sql` coverage for tenant-member read and tenant-admin write behavior. Remaining groups: `apprentice_placements`, `super_admin_action_audit`, `team_invitations`, `tenant_locations`, `xero_audit_log`, `xero_m2m_connections`. Continue one coherent policy group per PR. |
