...
| 2.2 RLS initplan rewrite | 25 direct-auth policies initially; Visual Feature Builder slice done, 9 direct-auth policies remain | 🟡 IN PROGRESS | CRM7 PR [#963](https://github.com/GaryOcean428/crm7/pull/963) rewrites the Visual Feature Builder policy group to `(SELECT auth.uid())` initplan wrappers and adds `09_feature_builder_rls_initplan.sql` pgTAP coverage for policy text plus owner/non-owner/platform-admin behavior. Remaining direct-auth groups: `apprentice_placements`, `role_capabilities`, `super_admin_action_audit`, `team_invitations`, `tenant_locations`, `xero_audit_log`, `xero_m2m_connections`. Continue one coherent policy group per PR. |
