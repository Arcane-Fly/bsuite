...
| 6.2 — Create Vault RPC `email_integration_set_encrypted_token` in DB; migrate plaintext `access_token` + `refresh_token` in `email_integrations` | DB migration | P0 | ✅ DONE | Migration `crm7/supabase/migrations/20260423_add_email_token_vault_columns.sql` adds Vault columns; edge fn `crm7/supabase/functions/encrypt-email-tokens/index.ts` migrates plaintext rows via `vault.secrets` RPC and nullifies source columns (lines 123-147) |
