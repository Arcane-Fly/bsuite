...
## 1. NOW (active / unblocked, highest value)

### Task: conduit recruitment intake chain (#219 → #218 → #231)
- **Owner (DRY):** conduit (recruitment); #231 cross-reads CRM7.
- **Description:** #219 candidate schema (APP consents + working rights + EIS demographics) → unblocks #218 public in-portal apply form (anon capture + dup detection) → #231 expand conduit→crm7 handoff snapshot (full candidate+application tree).
- **Rationale:** completes the front of the recruitment funnel; #231 is the DRY handoff to CRM7 placements.
- **Dependencies:** #218/#231 depend on #219.
- **Acceptance:** candidate schema migration live + Zod/TS mirrors; public apply form submits + dedups; handoff snapshot carries the full tree and CRM7 ingests it.
...
- **Skills:** `best-practice-research`, `dry-one-shot-architecture`, `supabase`. **Complexity:** High. **Status:** Not started.
...
2. DB → floor-gated migration (≥`20260611000000`) via the `supabase-migrate.yml` psql pipeline; verify live catalog; `get_advisors` triage + allowlist new SECURITY DEFINER fns.
3. User-facing → §12.3 deployed signed-in verify on `d.*`/prod via BrowserBase (test creds: `braden.lang77@gmail.com`); seed REAL data and exercise the populated path ([[feedback_prod_validation_catches_empty-data_bugs]]).
4. Cross-app reads (DRY one-shot) — owning app provides CRUD; others read + link.
5. Parent pointers don't auto-track app dev→main; reflect via `update-index` + commit index (no pathspec) per [[feedback_parent_pointer_reconcile_gotchas]].
6. Maker → independent Verifier → merge; `--merge` not squash for sync PRs.
