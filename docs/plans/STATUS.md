...
| [#1237](https://github.com/GaryOcean428/bsuite/issues/1237) | Reports W2 Uplift — Task 6 RPC schema-gap closure | 🔄 In progress | Tasks 6.1, 6.2, and 6.3 shipped via `crm7#956` (`report_rejected_timesheets` + pgTAP + grant hardening), `crm7#957` (`report_hours_by_work_type` from entries JSONB), and `crm7#958` (`report_pay_item_group_hours` hours-first pay item group mapping). Remaining pay-items-by-employee, CoInvest LSL, and consultant KPI RPCs stay blocked on the schema decisions documented in the issue. Plan: `20260521-reports-w2-uplift-implementation-v1.00W.md`. |
