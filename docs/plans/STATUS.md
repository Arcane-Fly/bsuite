...
| Package blockers | `charge-calc` is now published and rolled out. Source versions ahead of npm were still observed for `schema-builder` (0.7.2 vs 0.7.0), `schema-registry` (0.3.5 vs 0.3.2), `theme` (0.4.1 vs 0.3.3), and `ui` (0.3.0 vs 0.2.0). App package manifests in the active checkout use npm semver, with no `workspace:*` or `file:../packages/*` matches. | Later package work must publish source-of-truth versions before consumer rollout. |
| Auth/security | `AUTH_CANONICAL.md` remains the authority: BS OAuth 2.1 PKCE + JWKS only; no cookie SSO. `@bsuite/auth@0.2.5` now adds cross-tab BS token removal handling and refresh-rejected expiry events, and all six consumers are on 0.2.5. | Continue Task 2 with Braden `getClaims()`/role lookup replacement, forbidden-pattern CI guard, RLS/client_id checks, Conduit service-role RPC grant verification, and browser smoke. |
