...
| Package blockers | `charge-calc` is published/rolled out. Task 7 package publish slice published `@bsuite/ui@0.3.0`, `@bsuite/schema-builder@0.7.2`, and `@bsuite/theme@0.4.1`; `@bsuite/schema-registry@0.3.5` remains npm-permission blocked in bsuite#1432 (`npm publish` 404/not found or no permission). Task 8 merged `@bsuite/ui@0.4.0` shared primitives + Storybook source to `main` in bsuite#1451 and mirrored it to `development` in bsuite#1453, but npm publish is blocked by bsuite#1452 because this runtime has no npm auth (`npm whoami` E401; `npm publish` E404/no permission), so the registry still serves 0.3.0. App package manifests in the active checkout use npm semver, with no `workspace:*` or `file:../packages/*` matches. | Fix npm permissions/trusted publisher for schema-registry and ui; publish `@bsuite/ui@0.4.0`; then update consumers whose lockfiles should pin newer package versions. |
...
| DB/source-of-truth | CRM7 open issues #831 and #758 remain the DB source-of-truth/migration-repair blockers. Parent `bsuite` now has a schema-control-plane implementation in progress: `supabase/migration-scopes.json`, generated `supabase/schema-manifest.json`, `scripts/supabase/*`, `supabase-preview-db.yml`, and schema-builder migration scope wired into `supabase-migrate.yml`. | Task 3 reconciles migrations, pgTAP, advisors, and live SECURITY DEFINER/RPC grants; operator issue bsuite#1460 connects native Supabase branching + Vercel preview env aliases. |
...
| Supabase schema topology | One shared Supabase backend (`tuybltdrdefjblnplpqo`) is fed by migrations across parent + submodules/packages. Parent `bsuite` owns the CI connection via `.github/workflows/supabase-migrate.yml`; CRM7 holds the bulk of schema definitions. The parent schema manifest now tracks 8 migration scopes (root, six app repos, and `packages/schema-builder`) with 471 SQL migrations and 60 function directories. Braden's `supabase/config.toml` points at stale/local project `iykrauzuutvmnxpqppzk`, so manual links from Braden must explicitly pass `--project-ref tuybltdrdefjblnplpqo`. | Use Supabase MCP `apply_migration` as the preferred audited production applier; CI `migration up --include-all` is the fallback/PR path; never run local `supabase db push` against production. Enable parent Supabase GitHub branching/Vercel aliases via bsuite#1460 for preview DB builds. |
