...
| WS-6 | AVETMISS / NCVER Export | 🟡 **MOSTLY DONE — FULL NAT FORMATTERS VERIFIED** | `crm7/src/lib/avetmiss/` now contains the full mandatory 9-file AVETMISS suite (`formatNat00010.ts`, `formatNat00020.ts`, `formatNat00030.ts`, `formatNat00060.ts`, `formatNat00080.ts`, `formatNat00085.ts`, `formatNat00090.ts`, `formatNat00120.ts`, `formatNat00130.ts`) + `stateVariants.ts` (WA/NSW/QLD/SA helper coverage); edge fn `crm7/supabase/functions/avetmiss-export/` generates/zips all 9 files; migration `20260423180000_ws6_avetmiss_fields.sql` adds Training Contract Identifier. Evidence: formatter/export wiring landed in `crm7#770`; `pnpm vitest run src/lib/avetmiss/__tests__/formatters.test.ts` and `pnpm run typecheck` passed on 2026-06-04. Remaining WS-6 work is lodgement validation/export hardening and state STA extract depth, not missing formatter files. |
...
**Evidence update 2026-06-04:** the earlier "NAT00020/30/60/80/85 missing" finding was stale. Those formatter files, their unit coverage, and edge-function generation paths landed in CRM7 via `crm7#770` (`c43e9b3e8b207cf175950ae7a2337603e27526c8`). Validation on CRM7 `development` passed `pnpm vitest run src/lib/avetmiss/__tests__/formatters.test.ts` (49/49) and `pnpm run typecheck`. WS-6 remains open for lodgement validation fixtures, export UX/API hardening, and state STA extract completeness.
