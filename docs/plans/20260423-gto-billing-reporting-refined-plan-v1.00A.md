...
| WS-6 | AVETMISS / NCVER Export | 🟡 **MOSTLY DONE — FORMATTERS + EXPORT CONTRACT HARDENED** | `crm7/src/lib/avetmiss/` now contains the full mandatory 9-file AVETMISS suite (`formatNat00010.ts`, `formatNat00020.ts`, `formatNat00030.ts`, `formatNat00060.ts`, `formatNat00080.ts`, `formatNat00085.ts`, `formatNat00090.ts`, `formatNat00120.ts`, `formatNat00130.ts`) + `stateVariants.ts` (WA/NSW/QLD/SA helper coverage); edge fn `crm7/supabase/functions/avetmiss-export/` generates/zips all 9 files and now returns validation errors using the frontend `entityId` contract while failing explicitly on source-query errors instead of generating partial exports. Migration `20260423180000_ws6_avetmiss_fields.sql` adds Training Contract Identifier. Evidence: formatter/export wiring landed in `crm7#770`; edge validation contract/source-query hardening landed in `crm7#1002`; focused AVETMISS tests, CRM7 typecheck, edge-file lint, and local edge-runtime load checks passed on 2026-06-04. Remaining WS-6 work is lodgement validation fixtures and state STA extract depth, not missing formatter files or edge API contract mismatches. |
