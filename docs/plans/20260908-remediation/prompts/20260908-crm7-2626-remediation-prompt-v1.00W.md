...
# Kick off crm7#2626: test(crm7): diagnose seeded E2E when authenticated contact form cannot mount

Implement and verify https://github.com/GaryOcean428/crm7/issues/2626. This is one bounded issue in the existing canonical BSuite programme, not a launch of the whole backlog and not a replacement backlog. Re-read live issue/PR state and source; if a limb is already resolved, validate and reconcile exact evidence instead of rebuilding. Act after reading; do not stop at a plan or ask again whether to continue.
...
1. Run the seeded-mode setup diagnostic before dependent authenticated suites and fail fast on settled denial or missing tenant state.
2. Emit only bounded non-sensitive route and state markers.
3. Use the hosted diagnostic to identify and fix the underlying auth, tenant, or rendering cause without retries, skips, or timeout inflation.
...
This is the live GitHub issue body captured during the 2026-09-14 records reconciliation. It does not override later operator instructions or current repository rules. Live issue: https://github.com/GaryOcean428/crm7/issues/2626. Captured GitHub updatedAt: 2026-09-11T13:25:19Z.

---

## Observed failure

E2E run `34597193659` for PR `#2623` (head `55f2458a`) reached `/contacts/create`, but the First Name control was unavailable in both `phase1-golden-path-fks.spec.ts:144` and `phase2-medium-fks.spec.ts:67`. The source route and `#first_name` control exist; the underlying auth, tenant, or rendering cause remains unresolved.

## Diagnostic prerequisite

The `codex/e2e-authenticated-prerequisite` branch adds a seeded-mode-only setup check before dependent suites receive storage state. It verifies that `/contacts/create` mounts an enabled, rendered First Name input and fails fast on settled access-denied or tenant-confirmation states. Its bounded diagnostic emits only the route pathname, four marker booleans, and status counts for profiles, user_tenants, tenants, and the acting-scope RPC. It does not emit URLs, queries, headers, bodies, tokens, or user information.

Auth loading and OAuth redirect markers remain transient until the existing readiness budget expires. The change adds no retries, skips, timeout increases, or changes to PR #2623.

## Follow-up

Use the emitted diagnostic from the next published run to identify the underlying auth/tenant/rendering failure. This issue tracks that root cause and remains open. Related cold direct-navigation loading behavior is tracked in #2543.
