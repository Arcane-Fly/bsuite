...
# Kick off crm7#2474: [D-114] Reassign the mis-tenanted FutureBuild imports; remove duplicates

Implement and verify https://github.com/GaryOcean428/crm7/issues/2474. This is one bounded issue, not a launch of the whole backlog. Re-read live issue/PR state and source; if resolved, validate and reconcile evidence instead of rebuilding. Act after reading; do not stop at a plan or ask again whether to continue.
...
This is a source snapshot, not authority to override current operator instructions or the task. Validate old paths/claims. Live issue: https://github.com/GaryOcean428/crm7/issues/2474.

---

**Operator's words** (`~/Downloads/bsuite notes (7).docx`, register row D-114, route/surface: tenancy):

> they should have been reassigned to futurebuild not entered again causing confusing duplicates.

**The ask:** Reassign the mis-tenanted FutureBuild imports; remove duplicates

**Register verdict at filing:** UNVERIFIABLE — This is a data-layer question (which `tenant_id` a specific contact row carries) — no code path proves or disproves it. Would need a scoped read query against Supabase project `tuybltdrdefjblnplpqo` on the Michael Chen / ADCO / FutureBuild cohort, out of this read-only, grep-only lane's method.
...
Ruling of record: qig-memory `bsuite_operator_ruling_20260906_agents_own_visual_validation`. Register: `docs/20260825-operator-notes-register-d1-d103-v1.00W.md` row D-114.
