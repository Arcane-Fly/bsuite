...
# Kick off conduit#711: e2e: authenticated coverage needs a non-production database; the target is an operator decision (Supabase branching OFF by ruling)

Implement and verify https://github.com/GaryOcean428/conduit/issues/711. This is one bounded issue in the existing canonical BSuite programme, not a launch of the whole backlog and not a replacement backlog. Re-read live issue/PR state and source; if a limb is already resolved, validate and reconcile exact evidence instead of rebuilding. Act after reading; do not stop at a plan or ask again whether to continue.
...
- **auth-oauth-local-testing** — load `/home/braden/.agents/skills/auth-oauth-local-testing/SKILL.md` and apply only its relevant scope.
- **bsuite-conduit-deploy-testing** — load `/home/braden/.agents/skills/bsuite-conduit-deploy-testing/SKILL.md` and apply only its relevant scope.
...
1. Obtain the operator-owned non-production target decision without creating paid resources by inference.
2. Run all gated authenticated a11y tests with zero gated skips and a real signed-in shell.
3. Fail closed against production and dispose only per-run database identities/resources created by the chosen design.
...
This is the live GitHub issue body captured during the 2026-09-14 records reconciliation. It does not override later operator instructions or current repository rules. Live issue: https://github.com/GaryOcean428/conduit/issues/711. Captured GitHub updatedAt: 2026-09-11T03:43:32Z.

---

> **Corrected 2026-09-11.** The original body ([preserved verbatim here](https://github.com/GaryOcean428/conduit/issues/711#issuecomment-5628951898)) said this was "an adoption gap, not an open design question" and that only three repository secrets were missing. Both were wrong; see [the correction](https://github.com/GaryOcean428/conduit/issues/711#issuecomment-5627951193). **Nothing in this issue authorises creating Supabase branches, projects or any other paid resource.**

## Purpose

conduit's e2e job proves the public surface but not the authenticated one. The gated-route a11y assertions in `tests/e2e/wcag-aa.spec.ts` need a signed-in session against a database that is **not production**. Baseline: 1 failed / 8 skipped / 9 passed (e2e run 34499991674 on #710's head `a23f8dab3`). The guard added by conduit#708 keeps the gap a red check instead of a skip that reads as a pass, so #710 is blocked on this issue, correctly.

## Standing ruling

Supabase native branching stays **OFF** (operator, 2026-08-15, restated 2026-08-17 on [bsuite#1892](https://github.com/GaryOcean428/bsuite/issues/1892#issuecomment-5311079926); parent repository variable `SUPABASE_BRANCHING_REQUIRED=false`). Whether it also covers per-PR preview branches created through the CLI, as crm7 does, is the operator's to say.

## Decisions that are the operator's

1. **The database target:** (a) a CLI-created per-PR preview branch on `tuybltdrdefjblnplpqo`; (a2) native branching through the parent (declined by the ruling); (b1) a runner-local Supabase stack in the parent repo posting a status to conduit PRs; (b2) a runner-local stack inside conduit's `e2e.yml`; (c) a dedicated e2e project.
2. **The credential that target needs**, which differs by target (section 5 of the correction).
3. **The 47 existing preview branches** on the production project: delete, keep, or choose which; and whether crm7's branch cleanup is fixed before another repo copies it.
4. **For a database built from the production schema dump, the composition rule:** only what production's own migration runner would apply, all unrecorded files, or wait for conduit#713.

## Agent work that needs no decision

- Target-agnostic harness fixes on #710's branch: the session cookie name derived from the Supabase URL instead of hardcoded to production; a landing check before every axe audit, with off-origin navigation blocked; a runtime storage-state reader so test registration is identical in every process; corrected guard and comment text. Independently reviewed, then pushed to #710's branch as signed commit `8ffce9a` (2026-09-11).
- A feasibility study of option (b2), a runner-local stack built on the parent's production-schema rehearsal precedent (`supabase-migration-rehearsal.yml`), so the target decision rests on measured facts.
- No `e2e.yml` wiring, branch creation or credential use until the target is chosen.

## Acceptance criteria

- In CI the 8 gated-route a11y tests **execute** against the chosen non-production database (`0 skipped` for the gated block), and each reaches its route in the signed-in shell before axe runs.
- A fail-closed step refuses to run against the production project ref, proven by a positive control: pointed at production, it exits non-zero.
- No run writes to production. Any per-run database or identity is disposed of by the run that created it.
- The guard `gated-route a11y coverage is wired` passes because a real session exists, never because the check was relaxed.

## History

The dated comments below record how this was worked out, including claims later corrected. Read [the 2026-09-11 correction](https://github.com/GaryOcean428/conduit/issues/711#issuecomment-5627951193) before acting on any earlier comment.
