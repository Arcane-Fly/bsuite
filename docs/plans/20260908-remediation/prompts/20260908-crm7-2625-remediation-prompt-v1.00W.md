...
# Kick off crm7#2625: [Jodie] Inserts can reference another organisation's record ids; create_picklist_option offers platform picklists RLS then refuses

Implement and verify https://github.com/GaryOcean428/crm7/issues/2625. This is one bounded issue in the existing canonical BSuite programme, not a launch of the whole backlog and not a replacement backlog. Re-read live issue/PR state and source; if a limb is already resolved, validate and reconcile exact evidence instead of rebuilding. Act after reading; do not stop at a plan or ask again whether to continue.
...
1. Validate every referenced record ID belongs to the current tenant before insert, preferably with a database guard shared by all writers.
2. Do not offer platform picklists through a tenant mutation path; explain the correct authority path in user language.
3. Prove cross-tenant references fail and same-tenant inserts and permitted picklist operations still work.
...
This is the live GitHub issue body captured during the 2026-09-14 records reconciliation. It does not override later operator instructions or current repository rules. Live issue: https://github.com/GaryOcean428/crm7/issues/2625. Captured GitHub updatedAt: 2026-09-11T12:31:13Z.

---

Two leftovers from the crm7#2622 tool sweep (crm7#2615 pass 3, d621d1570). Neither lets one organisation read or change another's rows, which pass 3 closed.

**1. Inserts can reference another organisation's record ids.**
- Jodie insert tools set `tenant_id = context.tenantId`, but don't check that ids in the payload (person, placement, client and so on) belong to the same organisation.
- The row saved is organisation A's, but it can point at B's record. This is the cross-tenant reference class counted by `scripts/check-cross-tenant-references.mjs`, and the same shape as bsuite#2611.
- **Fix:** before each insert, check that every referenced id is in the chat's organisation (read it with a tenant filter, and refuse if absent). A database guard like the one for charge_rate_quotes would close it for every writer.

**2. `create_picklist_option` offers platform picklists it cannot change** (`src/lib/ai/tools/ui-builder-tools.ts:141-176`).
- The lookup matches `scope.eq.platform` picklists and refuses only `is_system` ones.
- Production RLS refuses the insert: `picklist_options_insert` requires the picklist's `tenant_id` to be one of the caller's owner/admin tenants, and a platform picklist's is NULL (pg_policies read 2026-09-11 12:30Z).
- So the tool fails with the raw database error and no explanation. That is a Jodie usability defect, not a cross-organisation write.
- **Fix:** exclude platform picklists from the lookup, or say "This list is shared by every organisation; ask a platform developer to change it."
