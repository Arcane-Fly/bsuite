# Kick off business-suite-unified#1237: No CI runs supabase/tests/database harnesses; two siblings bind all interfaces and use postgres:16

Implement and verify https://github.com/GaryOcean428/business-suite-unified/issues/1237. This is one bounded issue in the existing canonical BSuite programme, not a launch of the whole backlog and not a replacement backlog. Re-read live issue/PR state and source; if a limb is already resolved, validate and reconcile exact evidence instead of rebuilding. Act after reading; do not stop at a plan or ask again whether to continue.
...
1. CI discovers and runs every database harness and fails when none are found or a harness reports failure.
2. Bind test Postgres to loopback, use the production major version, and isolate temporary logs with cleanup.
3. Plant a failing scenario in a scratch copy and prove the CI contract turns red.
...
This is the live GitHub issue body captured during the 2026-09-14 records reconciliation. It does not override later operator instructions or current repository rules. Live issue: https://github.com/GaryOcean428/business-suite-unified/issues/1237. Captured GitHub updatedAt: 2026-09-11T07:34:25Z.

---

## What happens

No BSU workflow runs `supabase/tests/database/*.sh`. The database harnesses are the only executable proof of several security properties, and today they run only by hand:
- `run_workflow_run_start_subject_ownership.sh` (70 scenarios plus a bite against production's function body);
- `run_tasks_automation_queue_idempotency.sh`;
- `run_workflow_subject_context.sh`.

A regression in those functions would pass CI.

The two sibling harnesses also:
- bind all interfaces instead of 127.0.0.1;
- use postgres:16 while production runs 17;
- write fixed `/tmp` log paths, where mktemp is needed.

## Fix

- Add one CI job that runs every `supabase/tests/database/run_*.sh` on a loopback postgres:17 service container. It is not a required check at first, and it keeps the existing check names.
- The job fails if no harness is found, and it checks each harness's output, not only its exit code.
- Bring the two siblings in line with the new harness: 127.0.0.1 only, postgres:17-alpine, mktemp logs removed in a trap.
- Add a positive control: one planted failing scenario in a scratch copy must turn the job red.

Refs BSU#1233 (review follow-up F3).
