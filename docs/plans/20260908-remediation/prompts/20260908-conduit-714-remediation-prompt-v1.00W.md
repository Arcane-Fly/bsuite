...
# Kick off conduit#714: Browser-direct writes bypass every TypeScript permission gate — the #708 class needs a database-side role predicate

Implement and verify https://github.com/GaryOcean428/conduit/issues/714. This is one bounded issue in the existing canonical BSuite programme, not a launch of the whole backlog and not a replacement backlog. Re-read live issue/PR state and source; if a limb is already resolved, validate and reconcile exact evidence instead of rebuilding. Act after reading; do not stop at a plan or ask again whether to continue.
...
1. Direct viewer/employer PostgREST candidate PATCH/DELETE and direct dispatcher invocation are refused by database/server authority.
2. Owner/admin positive controls still work and affected-row counts prove silent RLS denials.
3. Close the confirm-dialog TOCTOU, user-scoped view-state, and remaining bulk-control gaps with bite tests.
...
This is the live GitHub issue body captured during the 2026-09-14 records reconciliation. It does not override later operator instructions or current repository rules. Live issue: https://github.com/GaryOcean428/conduit/issues/714. Captured GitHub updatedAt: 2026-09-10T12:52:52Z.

---

## The permission class cannot be closed in TypeScript for browser-direct writes

conduit#708 swept the permission-gate class across middleware, server actions, an edge function and RLS. That sweep was **structurally incomplete**, and not because instances were missed — because it counted the wrong layer.

Found during two-pass adversarial verification of the review findings on #707, each pass prompted to refute the other.

### The mechanism

`updateCandidate` and `deleteCandidate` write from the **browser** through `createBrowserClient` straight to PostgREST, carrying the user's own JWT:

- `src/lib/stores/candidateStore.ts:126,135,177`
- `src/lib/supabase/client.ts:19-32`
- `deleteCandidate` at `:177-178` is `.delete().eq('id', id)` — **not even tenant-scoped in the client**, relying wholly on RLS

**There is no server hop to gate.** `src/proxy.ts` middleware never sees a `supabase.co` request. So a TypeScript permission check can only decide whether the *control is offered*; it cannot refuse the *actor*. Anyone who opens a console can issue the same `PATCH`/`DELETE` with their own session.

The branch's own comment at `_canEditCandidates.ts:57-62` already says this. It was right, and the class sweep did not act on it.

### Same shape, different door: the shared edge function

`email-dispatcher` is invokable from a browser console with the public anon key and any member's JWT. It verifies the JWT and `user_tenants` membership (`:615`, `:682-698`) and **reads no role at all**. So a tenant member who cannot see the Communications button can still dispatch SMS and email by calling it directly.

That function is shared by **crm7, BSU, conduit, braden and R80.4**, and `ConduitRole` is conduit-local vocabulary. The check therefore has to be expressed in `portal_role` terms and designed **once for five callers** — not patched per app.

### What actually closes it

A **database-side role predicate** on the `r7_candidates` UPDATE and DELETE policies — the same shape as `check_user_portal_role(uid, tenant, roles[])`, which gates `portal_role` rather than `user_tenants.role` (those are different columns and a role in the other one is invisible to a single-column query).

**Read #713 first.** It measured that 4 of 5 live `r7_candidates` policies have **no counterpart in `supabase/migrations/`**, and `r7_current_tenant_id()` exists nowhere live despite ten migration files referencing it. Writing a new policy against the migrations directory would target a schema that is not deployed.

### Honest severity: low today, and that is an accident

Reachable **nominally** by any active tenant member. Reachable **in practice today by nobody** — the live population was enumerated across *both* `role` and `portal_role`, and the only under-privileged accounts are one `host_contact` and one `apprentice`, both in a tenant with zero `r7_candidates` rows, zero active `message_numbers` and zero `email_integrations`. Every member of the one SMS-capable tenant is an `owner` → `conduit_admin` → `ALL_PERMISSIONS`.

**That is a population accident, not a control.** It becomes live the first time someone adds a `host_contact` or `apprentice` to a tenant that holds candidates. The first verification pass priced this as `high`/`medium`; the second corrected it to `low-as-reachable, medium-as-latent` and noted the first "priced a population it never enumerated". The lower number is the honest one, and the reason it is low is worth reading before anyone relies on it.

### Also carried here, smaller and TypeScript-closable

- **`handleDelete` TOCTOU** — `canDelete` is checked before `requestConfirm` opens, but the stored async confirm callback calls `deleteCandidate(candidate.id)` without rechecking. Role change while the dialog is open still deletes. (The store operation has no role check either — see above.)
- **`viewStateStorageKey` is tenant-only, not user-scoped** — sign-out then sign-in as a different user in the same tab inherits the previous user's selected candidate IDs, immediately usable for bulk actions. Fix is a key-shape change. Note the inherited IDs are rows the second user is legitimately entitled to, so this is a UX/correctness leak rather than a privilege escalation — tenant-only RLS here is the deliberate design (`20260304040000:88-93`).
- **Two ungated bulk controls** at `_view.tsx:996` and `:1004` want the same `canEdit` gate as their siblings.

## Acceptance criteria

- A direct PostgREST `PATCH` and `DELETE` against `r7_candidates`, issued with a `viewer`/`employer`-`portal_role` session JWT and no server hop, is **refused by the database** — proven with the affected row count, never the exit code (an RLS-denied UPDATE/DELETE is **silent** and returns `UPDATE 0`).
- A direct `functions.invoke('email-dispatcher', …)` with the same session is refused on role, and the refusal is expressed in `portal_role` terms so all five consuming apps inherit it.
- A positive control proves the predicate can fail: the same call as an `owner`/`admin` still succeeds.
- The TOCTOU and storage-key items have unit coverage that fails without the fix.

## Mandatory before merge (FF-SELF-VALIDATION-20260507)

- **Validation loop**: §9.1 output-equivalence — baseline is today's silent success; target is a refusal with a row count of 0 and an error for INSERT.
- **Equivalence target**: affected-row counts for each role against each policy, before and after.
- **Cross red-team**: claude-code verifies the positive control genuinely bites before flip-to-done.
- **Skills to load**: `bsuite-rls-authz-red-team`, `auth-supabase`, `check-security`
- **Self-report on divergence**: yes.

## Blocked on

**#713 must land first.** A policy change written against the migrations directory would target a schema the database does not have.
