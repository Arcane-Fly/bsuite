...
# Kick off bsuite#3231: [P1][UX] Collapsed left navigation is broken (labels clipped/wrapping in icon mode) across CRM7/BSU/braden sidebars; whole-page visual acceptance

Implement and verify https://github.com/GaryOcean428/bsuite/issues/3231. This is one bounded issue in the existing canonical BSuite programme, not a launch of the whole backlog and not a replacement backlog. Re-read live issue/PR state and source; if a limb is already resolved, validate and reconcile exact evidence instead of rebuilding. Act after reading; do not stop at a plan or ask again whether to continue.
...
- **bsuite-shared-ui-rollouts** — load `/home/braden/.agents/skills/bsuite-shared-ui-rollouts/SKILL.md` and apply only its relevant scope.
...
1. Collapsed navigation hides labels deliberately, centers usable icon targets, and preserves accessible names/tooltips.
2. Verify expanded and collapsed states across affected app shells at required widths and both themes.
3. Fix the shared primitive/shell class and retain lossless navigation state.
...
This is the live GitHub issue body captured during the 2026-09-14 records reconciliation. It does not override later operator instructions or current repository rules. Live issue: https://github.com/GaryOcean428/bsuite/issues/3231. Captured GitHub updatedAt: 2026-09-09T03:44:11Z.

---

## Finding (supervisor visual acceptance, 2026-09-09)

Observed on d.crm.crm7.app (development sha c85168090) in the Claude desktop browser pane at ~800px viewport, light and dark themes: the **collapsed left navigation is visibly broken**. It looks like the expanded sidebar squeezed into a narrow strip — label text stays visible, clipped and wrapping beside/beneath icons; icons and labels do not form consistent centred targets; the active destination is hard to distinguish. Expanded state at the same width is readable (labels, grouping, active state OK).

DOM evidence (collapsed): wrapper `data-state="collapsed" data-collapsible="icon"`, sidebar width 47px, menu buttons 32px wide, but each label `<span>` still rendered (44–80px wide, `overflow: visible`); "Dashboard" item 70px wide inside a 47px rail.

This fails visual acceptance even though every link works. Whole-page visual acceptance (navigation, layout, theme, D2C tokens, responsive) is part of the standard for every UI closeout.

## Root cause (crm7, confirmed in source)

`crm7/src/components/ui/sidebar.tsx` `SidebarMenuButton` hides the label in icon mode with `[&>span:last-child]:group-data-[collapsible=icon]:hidden` and sizes the button with `group-data-[collapsible=icon]:!size-8 !p-2`.

`crm7/src/components/layout/AppSidebar.tsx` renders collapsible sections as `<Icon/><span>{label}</span><ChevronRight …/>` — the span is not the last child, so the label is never hidden; the chevron is not hidden either. Plain links use `asChild` with `MemoTransitionNavLink` (`<Icon/><span/>`) and still measure 70px, so the size override is not landing on that element either. Hiding overflow would only conceal the symptom.

## Affected sibling surfaces (enumerate and verify each)

- crm7: `src/components/ui/sidebar.tsx` (primitive), `src/components/layout/AppSidebar.tsx`, `src/layouts/MainLayout.tsx` — all CRM7 routes.
- business-suite-unified: `src/components/layout/AppSidebar.tsx` (own ChevronRight pattern), `src/components/AppContent.tsx`.
- braden: `src/components/ui/sidebar.tsx`, `src/components/admin/AdminLayout.tsx` (admin).
- conduit: `src/components/DashboardShell.tsx` via `@bsuite/ui` AppShell — verify whether a collapsed rail exists there.
- throughput: `src/components/layout/MainContent.tsx` via `@bsuite/ui` AppShell — same.
- `packages/ui/src/app-shell.tsx` (@bsuite/ui) — shell consumer; confirm whether nav primitives should move here (do not introduce a competing shell).

## Requirement

Implement a deliberate icon-only collapsed state: visual labels fully hidden (not clipped), centred icons, consistent click targets, clear active/hover/focus states, accessible names plus tooltips on hover **and** keyboard focus. Expanding must restore readable labels without clipping or overlapping the page. Verify collapsed and expanded at desktop and narrow widths, both themes, with usable screenshots per consuming app. Use semantic `--role-*` tokens; no palette-bound colours. Fix the class (primitive), not one page.

## Related responsive observations on /communications (~590px content width)

- Mailbox rail overlaps the message list ("New message" button over the search box; rail labels truncated with badges over list rows).
- KPI strip labels truncate to "Unr…/Sen…/Deli…/Fail…"; on first paint the cards briefly cascade/overlap before the grid settles (visible flash on load; also seen on /workflows/:id).
These are separate from the sidebar fix but must be dispositioned before the crm7#2594 D8 gate.

Programme: bsuite#3204. Discovered during crm7#2594 live verification. Supervisor: Claude Desktop Fable 5.1 (https://claude.ai/code/session_01FS8UfVhzkWXdn72Ko9zM2a).
