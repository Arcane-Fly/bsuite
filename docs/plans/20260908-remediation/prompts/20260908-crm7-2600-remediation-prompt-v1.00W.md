...
# Kick off crm7#2600: [P1][UX] /communications mail client panes overlap at narrow widths; KPI labels truncate; first-paint grid cascade (blocks crm7#2594 C4/D8)

Implement and verify https://github.com/GaryOcean428/crm7/issues/2600. This is one bounded issue in the existing canonical BSuite programme, not a launch of the whole backlog and not a replacement backlog. Re-read live issue/PR state and source; if a limb is already resolved, validate and reconcile exact evidence instead of rebuilding. Act after reading; do not stop at a plan or ask again whether to continue.
...
- **bsuite-page-grid-layout** — load `/home/braden/.agents/skills/bsuite-page-grid-layout/SKILL.md` and apply only its relevant scope.
...
1. Communications panes collapse in a deliberate order and never overlap or clip labels/badges.
2. KPI cards reflow before text truncates and the shared page-grid avoids first-paint overlap.
3. Verify the whole page at 768, 1024, and 1440 with expanded/collapsed navigation and both themes.
...
This is the live GitHub issue body captured during the 2026-09-14 records reconciliation. It does not override later operator instructions or current repository rules. Live issue: https://github.com/GaryOcean428/crm7/issues/2600. Captured GitHub updatedAt: 2026-09-11T06:36:18Z.

---

## Finding (operator screenshot + supervisor DOM measurement, 2026-09-09, d.crm.crm7.app @ c85168090)

On `/communications` at a content width of roughly 590–760px (expanded sidebar in a ~1000–1200px window, or any narrow desktop/tablet-landscape viewport), the mail client renders with **clear visual overlaps**:

- The mailbox/views rail overlaps the message list: the "New message" button is drawn under the "Search inbox" field; rail labels truncate ("All mailbo…", "braden@b…") and their count badges sit on top of list rows; list sender/subject text is clipped ("Brade…", "Raha…").
- The KPI strip labels truncate to "Unr…/Sen…/Deli…/Fail…" while the cards keep a four-column layout.
- On first paint the KPI cards briefly cascade/overlap each other before the grid settles (visible layout flash on every load; also seen on `/workflows/:id` where the "Every step" card momentarily overlaps "Triggers & Automations").
- The sidebar's "Portal" item icon is clipped at the bottom edge of the navigation scroll area.

Evidence: `.smoke-artefacts/crm7-communications-overlap-operator-20260909.png` (operator screenshot, copied to the protected remediation evidence dir) and supervisor DOM notes in the crm7#2594 live-verification record. Both themes affected; the dark theme itself renders coherently.

## Why this blocks crm7#2594 C4 / D8

C4 requires a *usable* CRM conversation surface with responsive layout verified; the reading-pane reply journey works, but the surface fails the whole-page visual gate at these widths. This is a page-layout class defect (page-grid card sizing + the three-pane mail client's minimum widths), not the SMS controls themselves.

## Requirement

- Three-pane mail client: define minimum pane widths and a deliberate collapse order (rail → icon rail or drawer; list → single-column with reading pane as an overlay/route) so panes never overlap; no clipped labels; badges never overlay another pane.
- KPI strip: reflow to 2×2 (or a horizontal scroll strip) before labels truncate; labels must stay readable.
- Page grid: no first-paint cascade — compute initial card heights before layout, or render the grid only after measurement (fix the class in the DraggableCardPage/page-grid layer; `/workflows/:id` is a sibling).
- Verify at 768px, ~1024px with sidebar expanded and collapsed, and 1440px, both themes, with usable screenshots. Use semantic `--role-*` tokens; no palette-bound colours.

Related: bsuite#3231 (collapsed navigation). Part of crm7#2594 (do not close it). Programme bsuite#3204. Supervisor: Claude Desktop Fable 5.1 (https://claude.ai/code/session_01FS8UfVhzkWXdn72Ko9zM2a).
