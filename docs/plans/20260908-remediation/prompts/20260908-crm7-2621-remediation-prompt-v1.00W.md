...
# Kick off crm7#2621: communications.is_read has no default in crm7's migration chain (production's comes from BSU 20261127); crm7-only replays fail outbound inserts

Implement and verify https://github.com/GaryOcean428/crm7/issues/2621. This is one bounded issue in the existing canonical BSuite programme, not a launch of the whole backlog and not a replacement backlog. Re-read live issue/PR state and source; if a limb is already resolved, validate and reconcile exact evidence instead of rebuilding. Act after reading; do not stop at a plan or ask again whether to continue.
...
1. Add a forward migration that gives communications.is_read a replay-safe default without editing applied history.
2. Replay crm7's migration chain independently and prove outbound inserts that omit is_read succeed.
3. Resolve outbound read-state semantics explicitly and verify production-equivalent behavior.
...
This is the live GitHub issue body captured during the 2026-09-14 records reconciliation. It does not override later operator instructions or current repository rules. Live issue: https://github.com/GaryOcean428/crm7/issues/2621. Captured GitHub updatedAt: 2026-09-11T10:27:42Z.

---

**Summary:** `communications.is_read` has no default in crm7's own migration chain. Any database built from crm7's `supabase/migrations` alone (crm7 preview branches, local stacks) ends up with `is_read boolean NOT NULL` and no default. So an insert that omits `is_read` fails, and the outbound writers do omit it (`buildCommunicationPayload` and the server-side outbound inserts).

**Production is not affected.** It has `is_read boolean NOT NULL DEFAULT false` (information_schema, read-only, 2026-09-11 09:44Z). The default comes from business-suite-unified `20261127000000_sms_inbound_durable_persist.sql:15-16` (`ALTER COLUMN is_read SET DEFAULT false`, "so inserts cannot fail the NOT NULL"), which is recorded as applied. crm7's `20261126000000_communications_inbound_read_state_and_provider_key.sql:26-59` adds the column, backfills it, and sets NOT NULL without a default.

**Found by:** Qodo on crm7#2620, a main parity backport that adds 20261126000000 unchanged. The same file is on development.

**Fix:** a forward crm7 migration on development that runs `ALTER TABLE public.communications ALTER COLUMN is_read SET DEFAULT false;`. It is idempotent and a no-op in production. It then goes through the normal reviewed pipeline apply. Do NOT edit 20261126000000; it is applied and recorded (statements md5 aaecf72e…).
- **Test:** replay crm7's chain alone into a throwaway Postgres, then insert an outbound communication without `is_read`. It must succeed.
- **Decide as part of the fix:** should outbound rows default to read (`true`)? The column comment says outbound is always read, and the UI treats outbound as read regardless (mailItems.ts:65,99). Production's default is `false` for every direction. The cleaner end state is either a direction-aware default or the writers setting it explicitly.

Related: crm7#2594 (C4 read/unread), crm7#2620.
