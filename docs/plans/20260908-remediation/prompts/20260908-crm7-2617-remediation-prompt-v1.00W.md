...
# Kick off crm7#2617: [security] api/ai/chat.ts rate limit runs after the refusal gates; server does not return the tenant it answered for

Implement and verify https://github.com/GaryOcean428/crm7/issues/2617. This is one bounded issue in the existing canonical BSuite programme, not a launch of the whole backlog and not a replacement backlog. Re-read live issue/PR state and source; if a limb is already resolved, validate and reconcile exact evidence instead of rebuilding. Act after reading; do not stop at a plan or ask again whether to continue.
...
1. Rate-limit refused requests before expensive authentication, tenant, and entitlement work while retaining intentional tenant limits.
2. Return the server-resolved tenant with the response and reject mismatched tenant answers in the client.
3. Prove refused requests are counted and mismatched answers cannot be displayed or persisted.
...
This is the live GitHub issue body captured during the 2026-09-14 records reconciliation. It does not override later operator instructions or current repository rules. Live issue: https://github.com/GaryOcean428/crm7/issues/2617. Captured GitHub updatedAt: 2026-09-11T08:47:56Z.

---

**`api/ai/chat.ts` checks the rate limit only after its refusal gates, so refused requests are never counted.**

Found while fixing #2615 (static read of crm7 `origin/development`):
- `checkRateLimit` runs at `api/ai/chat.ts:864`.
- The 401, 409/503 and 402 refusals return earlier, at lines 338-847.
- A client that is being refused (unauthenticated, wrong organisation, over quota) can therefore repeat the request without limit, and every repeat still costs the auth, tenant-resolution and entitlement queries.

**Also, the same file never tells the client which organisation it answered for.** If another tab or device changes the organisation pin mid-conversation, this tab's chat can receive the other organisation's answer and save it. crm7#2615 fixed the client side of the scope; this part needs the server's help. The server should return the tenant it resolved, and the client should reject an answer for a tenant other than the one it asked about.

**Fix:**
- Run a per-IP or per-user limit before the expensive gates (keep the per-tenant limit where it is if it is intentional).
- Return the answered tenant id in the response headers or stream metadata.
- Have `useAIChat` refuse a mismatched answer.
- Tests: a refused request is counted, and a mismatched tenant is rejected on the client.
