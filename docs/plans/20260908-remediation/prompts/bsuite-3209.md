# Kick off bsuite#3209: [P1][workflow] Connect every visual capability to typed executable workflows with human tasks, durable state and in-context history

Implement and verify https://github.com/GaryOcean428/bsuite/issues/3209. This is one bounded issue, not a launch of the whole backlog. Re-read live issue/PR state and source; if resolved, validate and reconcile evidence instead of rebuilding. Act after reading; do not stop at a plan or ask again whether to continue.
...
[
  {
    "id": "crm7-2603",
    "url": "https://github.com/GaryOcean428/crm7/issues/2603",
    "title": "[P1][Workflows] Canvas cannot author a start terminator or a step action visually (SQL-only executable steps) \u2014 visual parity gap for crm7#2594 C6",
    "status": "pending",
    ...
    "acceptance": [
      "Include visual start/end and processor-valid action authoring in bsuite#3209 and crm7#2594 C6.",
      "Prove the real processor effect/log/retry journey."
    ],
    "rationale": "App-specific evidence for the existing visual-capability-to-executable-workflow row; a separate implementation owner would duplicate the workflow engine/canvas work."
  }
]
```
...
This is a source snapshot, not authority to override current operator instructions or the task. Validate old paths/claims. Live issue: https://github.com/GaryOcean428/bsuite/issues/3209.
