...
# Kick off crm7#2609: [P2][Integrations] Storage tab is a read-only stub — extend Microsoft/Google connections, surface configured Supabase buckets, fix header split/duplicate actions/nested glow (CODEX_STORAGE_UX_REVIEW_20260909)

Implement and verify https://github.com/GaryOcean428/crm7/issues/2609. This is one bounded issue in the existing canonical BSuite programme, not a launch of the whole backlog and not a replacement backlog. Re-read live issue/PR state and source; if a limb is already resolved, validate and reconcile exact evidence instead of rebuilding. Act after reading; do not stop at a plan or ask again whether to continue.
...
1. Inventory live Supabase buckets/policies and existing Microsoft/Google identities and scopes before design.
2. Represent platform, connected, scope-missing, and unconfigured storage states honestly.
3. Complete upload, browse/select, save/reload, permission, document, and workflow round trips with in-context recovery.
...
This is the live GitHub issue body captured during the 2026-09-14 records reconciliation. It does not override later operator instructions or current repository rules. Live issue: https://github.com/GaryOcean428/crm7/issues/2609. Captured GitHub updatedAt: 2026-09-09T08:32:14Z.

---

## Requirement (Braden, via CODEX_STORAGE_UX_REVIEW_20260909)

Braden reviewed the deployed **External Integrations → Storage** tab (`/settings/integrations`, tab `storage`, `src/pages/settings/integrations.tsx`) and says design/UI defects remain. The route is **not accepted** until live UX and persistence are verified; the honest "Read-Only Preview" warning is not feature completion.

### Visible defects (operator screenshot)

- Left navigation labels clipped at the viewport edge (shared cause: bsuite#3231).
- Heading and the red read-only warning are awkwardly split across the page header (the `Alert` lives in its own `CanvasCard w={6} h={6}` beside the title card).
- Excessive nested / glowing panels (shared causes: crm7#2113 box-in-box; crm7#2506 resting dark accent glow).
- Duplicate **Add Storage Service** actions (tab header button + empty-state button both rendered).
- Empty state claims "No Storage Services" while provider configuration may already exist elsewhere (Microsoft / Google app connections, Supabase buckets) — the UI must never infer "no buckets".
- Actionable-looking controls (Add / Edit / Set default / form Save) on a page whose warning says storage cannot save; the `create/updateDocumentStorageMutation` are read-only stubs.

### Product requirement

- Storage must **extend the existing Microsoft 365 and Google app connections** (reuse the existing OAuth infrastructure and provider identities; request extra scopes — Drive / SharePoint-OneDrive files — only where required) and **account for the Supabase storage buckets already configured** for the platform.
- Before design: inventory existing provider identities and scopes, tenant linkage, the document-storage abstraction(s) and their consumers (documents, workflows, attachments), bucket use, and the **LIVE** storage policies/grants (`storage.objects` policies, bucket public/private flags) — query live, not the migration files.
- Distinguish four states explicitly in the UI: **platform storage** (Supabase buckets), **tenant-enabled providers** (connected + accessible), **connected-but-inaccessible** (identity exists, scope/permission missing → in-context re-consent), and **genuinely unconfigured**.
- Wire browse / upload / select / save / reload / permissions and the document + workflow consumers through the existing architecture (canonical entities/stores/services; no competing storage engine), with in-context configuration and failure recovery.
- Enumerate sibling settings / nav / layout surfaces and fix the shared causes rather than the one page; verify both themes and 390 / 768 / 1024 / 1440.
- Related: crm7#2511 (V-C8 free-text entity inputs on this page), crm7#1705 (clients connecting their own Google/Azure/SMTP accounts — same identity/scope model), crm7#2113, crm7#2506, bsuite#3231.

### Acceptance

Live d.crm evidence at the deployed SHA: real provider connection (Braden's own Microsoft/Google identity, no fabricated tenants), bucket listing that matches `SELECT id, name, public FROM storage.buckets` live, upload → select → save → reload → permissions round trip, and a D8 report (`ui_touched=yes`, sibling count, usable controls, persisted state, intact round trip, action count, considered alternative). Both themes, four widths, no clipped nav labels, one Add action, no accent glow on resting dark cards.

Ownership: Claude Desktop supervisor (remediation ledger `hermes-queue.json` key `codex_storage_ux_review_20260909`); queued **after** the current production-compatibility promotions. Programme bsuite#3204; do not close crm7#2594 from this issue.

Supervisor session: https://claude.ai/code/session_01FS8UfVhzkWXdn72Ko9zM2a
