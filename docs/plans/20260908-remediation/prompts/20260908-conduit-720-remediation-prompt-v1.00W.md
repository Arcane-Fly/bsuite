...
# Kick off conduit#720: r7_communications FOR ALL policy lets any member delete an SMS claim row mid-flight, re-opening the duplicate-send race

Implement and verify https://github.com/GaryOcean428/conduit/issues/720. This is one bounded issue in the existing canonical BSuite programme, not a launch of the whole backlog and not a replacement backlog. Re-read live issue/PR state and source; if a limb is already resolved, validate and reconcile exact evidence instead of rebuilding. Act after reading; do not stop at a plan or ask again whether to continue.
...
1. Split r7_communications authority so ordinary members cannot update/delete in-flight claim or audit rows.
2. Inventory every legitimate writer before changing policies and preserve the authorized claim/settle path.
3. Prove member DELETE refusal, service/owner positive behavior, and no duplicate SMS after a mid-flight attack.
...
This is the live GitHub issue body captured during the 2026-09-14 records reconciliation. It does not override later operator instructions or current repository rules. Live issue: https://github.com/GaryOcean428/conduit/issues/720. Captured GitHub updatedAt: 2026-09-11T10:31:08Z.

---

**Summary:** `r7_communications` has one row-level-security policy, `FOR ALL`, that lets any active member of the tenant insert, update or **delete** any row. conduit#710 uses an `r7_communications` row as the single-flight claim for an interview SMS: one claim per intent key, so a retry cannot send twice. Any member who deletes or edits that claim row while a send is in flight re-opens the race: a second resend then goes to the provider. The independent review of #710 pass 3 reproduced it (scenario R4: claim row deleted mid-flight → 2 texts, with or without provider dedup).

**Severity:** low as a new capability. The same members can already edit `r7_candidates.phone` and template bodies. It is still a hole in the duplicate-send guarantee, and a claim record should not be writable by everyone who can read it.

**Fix (needs a migration, so it is out of #710's scope):** split the policy by command and role. Members SELECT and INSERT their own tenant's rows. UPDATE and DELETE of claim or audit rows are limited to the service path, or to owner and admin. Test that a member's DELETE of a claim row is refused, and that the send path still claims and settles. Check every other writer of `r7_communications` first (method and count), so no legitimate path loses write access.

Found by the #710 pass-3 review (evidence: `~/.agents/state/conduit-710-pass3-evidence/review/`, scenario R4). Related: conduit#710, business-suite-unified#1239 (a replayed send is charged twice).
