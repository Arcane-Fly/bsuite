# Kick off conduit#718: AwardSelector: a failed lookup tells the user the stored award code does not exist, and a code change briefly shows the previous award's name

Implement and verify https://github.com/GaryOcean428/conduit/issues/718. This is one bounded issue in the existing canonical BSuite programme, not a launch of the whole backlog and not a replacement backlog. Re-read live issue/PR state and source; if a limb is already resolved, validate and reconcile exact evidence instead of rebuilding. Act after reading; do not stop at a plan or ask again whether to continue.
...
- **web-forms-validation** — load `/home/braden/.agents/skills/web-forms-validation/SKILL.md` and apply only its relevant scope.
...
1. Represent award loading, error, not-found, and found states separately and never invite correction after a transient lookup failure.
2. Never pair an old resolved label with a new award code.
3. Use deferred-promise race/error/not-found tests and sweep sibling selectors for the same discarded-error pattern.
...
This is the live GitHub issue body captured during the 2026-09-14 records reconciliation. It does not override later operator instructions or current repository rules. Live issue: https://github.com/GaryOcean428/conduit/issues/718. Captured GitHub updatedAt: 2026-09-11T06:36:42Z.

---

## What happens

1. **A failed lookup reads as "no such award".** `src/components/entity/AwardSelector.tsx:61-68` (origin/development) destructures only `data` from `.maybeSingle()` and discards `error`. A transient backend or network failure therefore sets `resolved = null`, and the user is told the stored award code does not match any award and is prompted to "correct" a value that is actually right.
2. **A stale label is shown during a change.** When `value` changes from A to B, `resolved` still holds A's `{id, name}` until B's lookup settles. For that window the selector pairs A's award name with B's code. The `cancelled` flag (`:59`, `:70`) stops a late result for A from landing after B, but it does not clear A while B is pending.

## Why it matters

The stored award code drives pay rates. A false "does not match" invites a user to overwrite a correct code. This is a data-integrity issue on a selector, not only cosmetic.

## Evidence

- Source read at origin/development (Codex 36979b0f).
- `AwardSelector.test.tsx` mocks resolve immediately, so neither the error path nor the pending window is exercised.
- The same test failed intermittently under machine load during a pre-commit run on 2026-09-11 (`waitFor` took 1540ms against its 1s limit). It passes alone. That timing failure is separate from the two defects above, and its cause is still unproven.

## Fix

- Represent loading, error, not-found and found as separate states.
- On error: keep the stored value, say the award could not be loaded, and offer a Retry that re-runs only the lookup. Never show "does not match".
- On a value change: clear the resolved label until the lookup for the current value settles, and key the result to the value it was fetched for.
- Tests use deferred promises: an A→B change with B resolving first and A late; a rejected or error response; valid; not-found. Add a bite for each.
- Count the class: the same `maybeSingle`-discards-`error` shape in the other EntitySelector-derived selectors in conduit, and in crm7's sibling selector if one exists.
