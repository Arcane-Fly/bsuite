...
# Kick off business-suite-unified#1239: [billing] email-dispatcher charges the tenant again when Mobile Message replays an idempotent SMS send

Implement and verify https://github.com/GaryOcean428/business-suite-unified/issues/1239. This is one bounded issue in the existing canonical BSuite programme, not a launch of the whole backlog and not a replacement backlog. Re-read live issue/PR state and source; if a limb is already resolved, validate and reconcile exact evidence instead of rebuilding. Act after reading; do not stop at a plan or ask again whether to continue.
...
1. Read provider replay metadata and prevent a repeated idempotent send from creating a second tenant charge.
2. Preserve exactly one charge for a fresh success and zero charge for provider rejection or replay.
3. Verify any provider_message_id database uniqueness behavior and make billing reconciliation explicit.
...
This is the live GitHub issue body captured during the 2026-09-14 records reconciliation. It does not override later operator instructions or current repository rules. Live issue: https://github.com/GaryOcean428/business-suite-unified/issues/1239. Captured GitHub updatedAt: 2026-09-11T09:28:37Z.

---

**`email-dispatcher` charges the tenant again when Mobile Message replays an idempotent send.**

Found by conduit#710 pass 3. It checked the deployed dispatcher (v88, read-only) and Mobile Message's documentation.

**Provider contract.** Mobile Message's documentation for POST /v1/messages says a repeated request with the same `Idempotency-Key` returns *"the original response, byte for byte, without re-processing anything. No second batch is sent, no credits are deducted again, no new message IDs are generated,"* and adds an `Idempotency-Replay: true` header. Keys last 24 hours per API key.
- https://mobilemessage.com.au/api-documentation
- https://help.mobilemessage.com.au/api/api-idempotency-keys

**Defect** (business-suite-unified `origin/development`):
- `supabase/functions/_shared/sms-provider.ts` sends `Idempotency-Key` (around line 138) but never reads the `Idempotency-Replay` response header.
- `supabase/functions/email-dispatcher/index.ts:501-532` therefore treats a replay as a fresh send. It inserts a `message_usage` row per result with `charged_cents = perMessageCharge` for each `success`.
- So a retried send (a lost response, a double click, a conduit "Send the text now" retry) produces one SMS from the provider but can produce two billable usage rows for the tenant.
- Check first whether `message_usage` has a unique constraint on `provider_message_id`. If it does, the second insert fails and is only logged, which loses nothing but hides the event.

**Fix:**
- Have `sendSms` return a `replayed` flag from the response header.
- On a replay, do not insert a new charged row. Either record nothing, since the original row already exists for the same `provider_message_id`, or record an uncharged `replayed` marker, whichever the billing reconciliation expects.
- Tests: a replayed response creates no second charge, a fresh response charges once, and a provider 422 (same key, different payload) is not charged.
- Keep the existing rule that blocked and failed attempts are recorded uncharged.

Related: conduit#710 (the retry-safe resend that exposed this), #1233's ownership of the migration lane (this fix needs no migration unless a unique index is the chosen guard).
