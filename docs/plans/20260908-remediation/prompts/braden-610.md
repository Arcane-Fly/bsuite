# Kick off braden#610: [P1] Positioning drift: site still presents Braden as a GTO that employs/places apprentices — sweep routes, CMS+fallback, CTAs, metadata, JSON-LD, llms/robots (CODEX_BRADEN_POSITIONING_20260909)

Implement and verify https://github.com/GaryOcean428/braden/issues/610. This is one bounded issue in the existing canonical BSuite programme, not a launch of the whole backlog and not a replacement backlog. Re-read live issue/PR state and source; if a limb is already resolved, validate and reconcile exact evidence instead of rebuilding. Act after reading; do not stop at a plan or ask again whether to continue.
...
- **copywriting** — load `/home/braden/.agents/skills/copywriting/SKILL.md` and apply only its relevant scope.
- **bsuite-brand-system** — load `/home/braden/.agents/skills/bsuite-brand-system/SKILL.md` and apply only its relevant scope.
...
1. Every public route, CMS override/fallback, navigation/CTA, metadata, JSON-LD, llms, and robots surface describes Braden truthfully as a software/consulting vendor.
2. Preserve valid GTO customer terminology and stable URLs while replacing misleading employer/placement claims and CTAs.
3. Verify visible copy and rendered metadata across development and production at exact SHAs.
...
This is the live GitHub issue body captured during the 2026-09-14 records reconciliation. It does not override later operator instructions or current repository rules. Live issue: https://github.com/GaryOcean428/braden/issues/610. Captured GitHub updatedAt: 2026-09-09T07:16:39Z.

---

## Problem (operator feedback 2026-09-09, CODEX_BRADEN_POSITIONING_20260909)

The public site still reads as though Braden Group **is** a Group Training Organisation that employs, places and recruits apprentices — contradicting the corrected positioning already live on `src/pages/Index.tsx` (an Australian software company building workforce-management platforms and supported consulting **for** GTOs, RTOs, labour-hire and recruitment providers) and contradicting the software-consultation form on the same pages.

Verified 2026-09-09 (local `origin/development` 51681d2 and live www.braden.com.au — local source is evidence of defects, not proof of every deployed endpoint; the homepage was not exhaustively audited):

| Surface | Verbatim / evidence |
|---|---|
| `src/pages/apprenticeships.tsx:19,30,49` | "Launch your career with Braden Group's comprehensive apprenticeship programs…" + `EducationalOccupationalProgram` JSON-LD describing Braden as the program provider; "Benefits of Our Programs" (L123); "Apply Now" (L212). Live /apprenticeships shows the same copy. |
| `src/pages/recruitment.tsx` | placement metadata presenting Braden as the recruiter/placer |
| `src/pages/Contact.tsx` | apprenticeship-service metadata and contact choices from the old positioning |
| `src/components/Layout.tsx:67` | footer "Professional recruiting solutions for apprenticeships and traineeships"; nav labels from the old positioning |
| `src/components/SEOHead.tsx:54` | stale fallback description "Australian Group Training Organisation specialising in apprenticeships, traineeships, workforce recruitment, and compliance management." although the default at L34 is corrected |
| `src/components/SEOHead.tsx:140` | title "Braden Group — Australia's GTO Workforce Solutions" |
| `public/llms-full.txt:9,17` (live too) | "Braden Group is an Australian Group Training Organisation (GTO)…", "We employ apprentices and trainees directly…" |
| `public/llms.txt:5` | "…place apprentices and trainees into host workplaces… our placement programs" |
| `public/robots.txt`, contact choices | old positioning retained |

## Required outcome

1. **Sweep every reachable route** (sitemap 9 URLs + any newer routes, incl. /gto-software-australia #602), **CMS overrides AND static fallbacks** (#371), header/footer/nav, forms and CTAs, `<title>`/meta/OG/canonical, JSON-LD, `llms.txt`/`llms-full.txt`, `robots.txt`, and sister-app backlinks (#609/#607 canonical host). Every statement about Braden must describe a software/consulting company serving GTOs/RTOs/labour hire/recruitment providers. Customer activities (employing, hosting, placing apprentices) must not become claims about Braden itself.
2. **Keep valid GTO customer terminology** — do not blanket-delete "GTO"; the audience is GTOs.
3. **Replace misleading program/placement CTAs** ("Apply Now", "Benefits of Our Programs", "Launch your career…") with truthful product/service journeys (platform demo, consultation, product pages). **Preserve existing URLs** with relevant content or deliberate redirects — no silent deletion of routes/features.
4. **No invented claims**: no accreditation, placement numbers, staff/client counts or achievements.
5. JSON-LD: `EducationalOccupationalProgram`/placement-service types replaced by types that are true for a software/consulting vendor (Organization, SoftwareApplication/Product, Service, WebPage) with truthful fields only.
6. Consistency between CMS-managed copy and static fallback (#371): the same corrected text in both; document which surfaces are CMS-editable.

## Definition of complete

Fresh **development and production** route/copy/metadata evidence at the deployed SHA for every reachable route (rendered HTML title/meta/OG/JSON-LD + visible copy + footer/nav + llms/robots), consistent across CMS and static fallback, recorded in the current remediation plan; D8 (ui_touched=yes; siblings: all public routes, header, footer, forms, SEOHead, llms files), ship-all-apps applicability and ops-ship-close-out gates. Copy-editing/copywriting skills apply.

Related: #609 (Search Console discovery/SEO rendering), #607 (canonical host), #371 (CMS wiring + compliance-claim wording), #602 (/gto-software-australia landing). Part of the estate remediation programme (bsuite#3204). Does not close those issues.
