...
# Kick off crm7#2622: [P1][security] Jodie record_self_assessment looks up without a tenant filter and rewrites tenant_id: a developer acting as A can move tenant B's assessment into A

Implement and verify https://github.com/GaryOcean428/crm7/issues/2622. This is one bounded issue in the existing canonical BSuite programme, not a launch of the whole backlog and not a replacement backlog. Re-read live issue/PR state and source; if a limb is already resolved, validate and reconcile exact evidence instead of rebuilding. Act after reading; do not stop at a plan or ask again whether to continue.
...
1. Every Jodie tool read/write on a tenant table filters by the current tenant and never rewrites tenant_id on an existing row.
2. Enumerate the full tool class and prove a developer acting as tenant A cannot read or mutate tenant B data.
3. Retain same-tenant positive controls and bite each enforced tenant predicate.

FutureBuild is a real client and its data is preserved. Do not use, rewrite, delete, or manufacture client records as test fixtures.
...
This is the live GitHub issue body captured during the 2026-09-14 records reconciliation. It does not override later operator instructions or current repository rules. Live issue: https://github.com/GaryOcean428/crm7/issues/2622. Captured GitHub updatedAt: 2026-09-11T11:11:55Z.

---

**Summary:** Jodie's `record_self_assessment` tool can move another organisation's self-assessment into the chat's organisation.

**Where:** `src/lib/ai/tools/compliance-test-tools.ts:146-152` looks up `gto_self_assessments` by `clause_id` and `assessment_year` only, with no tenant filter. It then PATCHes the first row found (`:175`) with `tenant_id: context.tenantId` (`:170`).

**How it happens:**
- Row-level security lets a platform developer read and update every tenant's rows (`supabase/migrations/baseline/20260907_prod_baseline_schema_dump.sql:64135`, `:64142`).
- So a developer acting as tenant A who records an assessment for a clause and year that A has no row for, but tenant B does, rewrites B's row into A. That includes FutureBuild's rows.
- The unique key (tenant_id, clause_id, assessment_year) at `:37702` blocks this only when A already has a row.
- Ordinary staff hit the same path if their pinned organisation changes mid-turn: the lookup runs under the new pin, and the write stamps the chat's organisation.

**Exposure today:** production `gto_self_assessments` has **0 rows** (read-only, 2026-09-11 11:11:35Z), so no row has been moved yet. The defect is latent until the tool is first used.

**Same pattern elsewhere.** These read without a tenant filter but are not reachable today, because `view_financial` / `view_compliance` are granted to no role (`api/ai/chat.ts:244-321`):
- `pay-run-tools.ts:66-74`;
- `payslip-tools.ts:78-88`, `:144-150`;
- `compliance-test-tools.ts:104-109`.

**Fix:**
- Every Jodie tool lookup and write filters `tenant_id=eq.${context.tenantId}`. A write never changes an existing row's `tenant_id`; update by `id` and `tenant_id` together.
- List the tools under `src/lib/ai/tools/` that read or write tenant tables, with the method and count, and fix each one.
- **Tests:** a developer acting as A cannot read or modify B's row through any tool. A same-tenant write still works. Bite each fix by removing it and showing the test fails.

Found by the independent review of crm7#2618 (crm7#2615 pass 2); evidence is in `~/.agents/state/crm7-2615-pass2-evidence/review/`. The fix is folded into the #2615 lane's next pass.
