...
# Kick off business-suite-unified#1235: Live-run uniqueness keys on the event and omits the tenant: manual and automatic runs of one definition can both be live

Implement and verify https://github.com/GaryOcean428/business-suite-unified/issues/1235. This is one bounded issue in the existing canonical BSuite programme, not a launch of the whole backlog and not a replacement backlog. Re-read live issue/PR state and source; if a limb is already resolved, validate and reconcile exact evidence instead of rebuilding. Act after reading; do not stop at a plan or ask again whether to continue.
...
1. Define one-live-run uniqueness by tenant, definition, and subject rather than trigger-event spelling.
2. Refuse manual starts when any equivalent live run exists and name the existing run without touching preserved client data.
3. Prove cross-event collision refusal, cross-tenant independence, and same-tenant positive behavior.
...
This is the live GitHub issue body captured during the 2026-09-14 records reconciliation. It does not override later operator instructions or current repository rules. Live issue: https://github.com/GaryOcean428/business-suite-unified/issues/1235. Captured GitHub updatedAt: 2026-09-11T07:34:22Z.

---

## What happens

The "one live run per subject" rule is keyed on the wrong things:

1. **The event is part of the key.** The unique index `uniq_workflow_runs_live_per_subject` (`supabase/migrations/20261104000000_*:527-529`) and the start guard in `workflow_run_start(uuid,text,uuid)` both include the trigger event. A manual Start run and an automatic run of the SAME definition on the SAME subject use different events, so they never collide and both stay live:
   - **Live today:** FutureBuild's waiting run is `placement` / `placement.manual_start` (written 2026-09-08 by an agent proof through the service-role 6-arg engine). A UI Start run on that placement would record `placement.created`, which creates a second live run. Reproduced on a disposable database as scenario D4 in the BSU#1233 harness.
   - **Latent:** apprentices (`apprentice.created` from `20261104000000:1420` vs `apprentices.started` from a manual start) and communications (`sms.inbound` / `sms.inbound.unmatched` from `20261129000000:348-349` vs `communications.started`). No such live runs exist today.
2. **The tenant is not part of the key.** A live run in another tenant on the same definition, event, table and subject would block a legitimate start. 0 such runs exist live, and BSU#1233's function can no longer create one (independent review AG6).

## Fix (needs a product ruling first)

Recommended: a manual start refuses while ANY live run of that definition exists on the subject in the caller's tenant, whatever the event, with a plain message naming the existing run. Add `tenant_id` to the uniqueness key through a reviewed index migration: measure for collisions first, and rebuild CONCURRENTLY if the table has grown. Do not rewrite any stored run, and never touch FutureBuild's run without Braden's ruling (Codex inbox 36a989ec).

Harness: extend `supabase/tests/database/run_workflow_run_start_subject_ownership.sh` with the three cross-event cases and a cross-tenant case, each with a bite.

Refs BSU#1233 (review follow-ups F1 and F5).
