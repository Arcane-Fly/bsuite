...
# Kick off conduit#709: confirm_sta_email checks user_tenants.role while its own RLS checks portal_role — the two can disagree

Implement and verify https://github.com/GaryOcean428/conduit/issues/709. This is one bounded issue in the existing canonical BSuite programme, not a launch of the whole backlog and not a replacement backlog. Re-read live issue/PR state and source; if a limb is already resolved, validate and reconcile exact evidence instead of rebuilding. Act after reading; do not stop at a plan or ask again whether to continue.
...
1. Measure live role versus portal_role divergence before choosing the authority vocabulary.
2. Make the SECURITY DEFINER function, RLS, and client gate use the same deliberate role source.
3. Prove both disagreement directions plus allowed and denied controls after the forward migration.
...
This is the live GitHub issue body captured during the 2026-09-14 records reconciliation. It does not override later operator instructions or current repository rules. Live issue: https://github.com/GaryOcean428/conduit/issues/709. Captured GitHub updatedAt: 2026-09-10T05:09:18Z.

---

Found while gating `offers/lodgementActions.ts` for #708. Not introduced by that branch — pre-existing, and it needs a migration, so it is filed rather than fixed.

## The divergence

`confirm_sta_email` is `SECURITY DEFINER` and does its own role check by reading **`user_tenants.role`**:

```sql
-- supabase/migrations/20260725140000_confirm_sta_email_atomic.sql:37-44
SELECT ut.role INTO v_role FROM user_tenants ut WHERE ...
IF v_role NOT IN ('owner','admin') THEN
  RAISE EXCEPTION 'only owners and admins can confirm STA emails';
```

The two RLS policies it works alongside read **`portal_role`**:

- `sta_inbound_emails_admin_update` — `portal_role IN ('owner','admin')` (20260725090000:183)
- `tcsc_admin_insert` — same (20260725090000:233)

Those are different columns. `role` is the access tier; `portal_role` is what the app actually gates on, and it defaults to `staff` independently of `role`. So the `SECURITY DEFINER` function and the RLS beside it can reach opposite verdicts about the same user on the same write.

## Why it matters both ways

- A user with `role` = `admin` but `portal_role` = `staff` passes the RPC's check and is refused by the RLS.
- A user with `portal_role` = `admin` but a lower `role` is refused by the RPC while the RLS would have allowed them.

Because the function is `SECURITY DEFINER` it runs as the owner, and its own check is the *only* role gate on that path — the RLS does not re-apply to it. So the RPC's column choice is load-bearing, not redundant.

## Scope of this report

Only the check was read. No SQL was executed and no live `pg_policies` query was made — every claim here is about the migration source in this tree. The two columns are known to diverge in this estate (that is why `portal_role` exists at all), but I did **not** measure how many rows actually differ today. **That count is the first thing to establish**, because it decides whether this is latent or already biting.

## Suggested fix

Change the RPC to read `portal_role`, matching the RLS beside it and the app's own gating vocabulary. That is a one-line migration, but it is a **behaviour change for anyone whose two columns disagree**, so measure the affected rows before writing it.

Related client-side work landed on `fix/conduit-708-permission-gate-class` (not yet merged): `StaEmailsSection` Confirm/Reject moved off `manage_offers` onto an admin-tier gate reading `role`, because `manage_offers` is held by `recruiter` — i.e. portal manager and staff — while the server requires owner/admin. That gate deliberately reads `role` to mirror the RPC **as it stands today**. If this issue is fixed by moving the RPC to `portal_role`, that client gate must move with it, or the mismatch simply reappears on the other side.
