...
# Kick off bsuite#3261: workflow-canvas: compact layout keeps a fixed 288px inspector beside the diagram, leaving a phone canvas ~50–90px of diagram

Implement and verify https://github.com/GaryOcean428/bsuite/issues/3261. This is one bounded issue in the existing canonical BSuite programme, not a launch of the whole backlog and not a replacement backlog. Re-read live issue/PR state and source; if a limb is already resolved, validate and reconcile exact evidence instead of rebuilding. Act after reading; do not stop at a plan or ask again whether to continue.
...
1. Selected-step editing remains usable at phone widths without a fixed 288px side column.
2. Selection and unsaved edits survive inspector open/close and compact/wide layout changes, with accessible close and focus return.
3. Verify bounding boxes at 375, 390, 768, 1024, and 1440 in both themes before the stable package release.
...
This is the live GitHub issue body captured during the 2026-09-14 records reconciliation. It does not override later operator instructions or current repository rules. Live issue: https://github.com/GaryOcean428/bsuite/issues/3261. Captured GitHub updatedAt: 2026-09-11T06:35:29Z.

---

## What happens

When the canvas container is narrower than 800px, `@bsuite/workflow-canvas`'s compact layout moves only the palette. The inspector keeps a fixed 288px column beside the diagram whenever a step is selected. On a phone-width canvas (about 375px) that leaves roughly 50–90px of diagram. The minimap is clipped, and editing a step is barely usable. On short windows (under about 583px tall) the palette column can also lose its bottom few pixels.

## Why (origin/development d0972e65a; same in the published 0.3.0-rc.4 dist)

- `packages/workflow-canvas/src/components/canvasRegions.ts:37` sets `COMPACT_MAX_WIDTH_PX = 800`, and `:107` chooses `compact` below it.
- `packages/workflow-canvas/src/components/chromeClasses.ts:29-30` sets `WORKFLOW_REGION_INSPECTOR = 'relative min-h-0 w-72 min-w-0 shrink-0 empty:hidden'`. There is no compact variant; only the palette has one (`WORKFLOW_REGION_PALETTE_COMPACT`, `:31`).
- `packages/workflow-canvas/src/components/WorkflowCanvas.tsx:262` renders that inspector region in both layouts.
- Consumers cannot fix it: the region contract forbids placement classNames on chrome.

Two readers confirmed this independently in the installed rc.4 source: `dist/components/chromeClasses.js:23` and `WorkflowCanvas.js:126`. The crm7 rc.4 adoption review estimated the same behaviour from source. It has not been measured live yet.

## Required (before 0.3.1 stable)

- In compact layout, selected-step editing must be usable on a phone: a drawer or sheet over the diagram, or a stacked region, never a fixed side column.
- Selection and unsaved inspector edits survive opening and closing it, and survive a change between compact and regions layouts.
- Accessible return: a named close control, Escape, and focus back to the selected node.
- The wide layout stays exactly as it is.
- Tests at a 375px container (diagram keeps at least 80% of the width while editing), plus a real-browser bounding-box check at 375, 390, 768, 1024 and 1440 in both themes.

Blocks: @bsuite/workflow-canvas 0.3.1 finalisation (parent release #3236). Related: crm7-2594 canvas criteria, bsuite-3205 draft round trip.
