...
# Kick off crm7#2604: [P2][UX] /workflows/:id visual debt: inspector overlaps palette/toolbar (hides Publish), nodes stack on add, non-D2C buttons incl. 'Back to Workflows', oversized footer in condensed layouts

Implement and verify https://github.com/GaryOcean428/crm7/issues/2604. This is one bounded issue in the existing canonical BSuite programme, not a launch of the whole backlog and not a replacement backlog. Re-read live issue/PR state and source; if a limb is already resolved, validate and reconcile exact evidence instead of rebuilding. Act after reading; do not stop at a plan or ask again whether to continue.
...
1. Workflow inspector, palette, toolbar, footer, and navigation controls do not overlap at condensed widths.
2. New nodes receive usable non-colliding positions and canonical semantic controls.
3. Verify keyboard use, action counts, responsive layouts, and both themes on the deployed development SHA.
...
[
  {
    "id": "crm7-2613",
    "url": "https://github.com/GaryOcean428/crm7/issues/2613",
    "title": "Footer too large on mobile view.",
    "status": "pending",
    ...
    "acceptance": [
      "Retain the screenshot as D8 evidence for crm7#2604.",
      "Map landing-page audience/value copy to bsuite#3224 rather than inventing a third public-site programme."
    ],
    "rationale": "Sparse operator report combines the footer symptom already captured by crm7#2604 with landing-page purpose/copy already owned by the public-site programme."
  }
]
```
...
This is the live GitHub issue body captured during the 2026-09-14 records reconciliation. It does not override later operator instructions or current repository rules. Live issue: https://github.com/GaryOcean428/crm7/issues/2604. Captured GitHub updatedAt: 2026-09-09T11:46:52Z.

---

## Finding (operator screenshot + supervisor session, d.crm.crm7.app @ c85168090, 2026-09-09)

`/workflows/:id` fails whole-page visual acceptance in a condensed window (~1220px with sidebar expanded):

- **Overlapping elements**: the node inspector (Start / End → Name / Notes / Delete this step) renders on top of the palette ("Add to this workflow") and the canvas toolbar, hiding the **Publish** button and half of "Tidy up"; the palette descriptions run under the inspector fields. Newly added nodes stack on top of each other at the same position ("END" over "END") instead of being placed apart.
- **Non-D2C controls**: "Back to Workflows" is an unstyled full-width bordered button; toolbar pills (Undo/Redo/Tidy up/Publish/Full screen), palette entries and the red "Delete this step" outline do not use the BSuite button/`--role-*` token system consistently; the workflow node cards look amateurish next to the rest of the app. The same unstyled-button pattern appears elsewhere in CRM7 (enumerate siblings during the fix).
- **Footer**: the global footer is too tall for condensed screens, taking a large share of the viewport on a page that is a working surface.
- Transient first-paint overlap between "Triggers & Automations" and "Every step, as a table" cards (page-grid re-flow) — also recorded on crm7#2600.

Evidence: `.smoke-artefacts/crm7-workflow-canvas-visual-debt-operator-20260909.png` (operator) and supervisor DOM notes in the crm7#2594 live-verification record.

## Requirement
- Inspector, palette and toolbar must never overlap: define the canvas page layout for narrow widths (inspector as a side sheet or below-canvas panel; toolbar wraps; palette collapses to icons with tooltips), keyboard-operable, both themes.
- New nodes are placed at a free position (offset from the last node or at the viewport centre with collision offset) and selected.
- "Back to Workflows" and all page-level navigation/actions use the canonical BSuite button variants and semantic `--role-*` tokens (D2C theme source of truth in the project docs); no bespoke bordered buttons; consistent hover/focus/active states.
- Footer: compact variant on working surfaces / below a viewport-height threshold; must not exceed a defined max height in condensed layouts.
- Verify at 1024px, ~1220px with sidebar expanded, and 1440px, both themes, with usable screenshots; record action counts and one rejected alternative.

Not required before the current SMS slice merges, but must be dispositioned before the remediation series is declared complete. Related: bsuite#3231, crm7#2600, crm7#2603. Programme bsuite#3204. Supervisor: Claude Desktop Fable 5.1 (https://claude.ai/code/session_01FS8UfVhzkWXdn72Ko9zM2a).
