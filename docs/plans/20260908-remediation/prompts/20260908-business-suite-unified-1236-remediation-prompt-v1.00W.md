...
# Kick off business-suite-unified#1236: workflow_run_start lets host supervisors start runs on (and confirm) rows their SELECT policy hides

Implement and verify https://github.com/GaryOcean428/business-suite-unified/issues/1236. This is one bounded issue in the existing canonical BSuite programme, not a launch of the whole backlog and not a replacement backlog. Re-read live issue/PR state and source; if a limb is already resolved, validate and reconcile exact evidence instead of rebuilding. Act after reading; do not stop at a plan or ask again whether to continue.
...
1. Apply the same subject visibility predicate to workflow start that the subject table SELECT policy applies.
2. Return indistinguishable not-visible and not-found refusals without existence disclosure.
3. Test hidden and visible host-supervisor subjects plus unchanged ordinary-member behavior.
...
This is the live GitHub issue body captured during the 2026-09-14 records reconciliation. It does not override later operator instructions or current repository rules. Live issue: https://github.com/GaryOcean428/business-suite-unified/issues/1236. Captured GitHub updatedAt: 2026-09-11T07:34:23Z.

---

## What happens

The live SELECT policies on `contacts`, `people` and `placements` hide some rows from host supervisors (`NOT is_host_supervisor_of_tenant(...)`). `workflow_run_start(uuid,text,uuid)` checks only that the subject row belongs to the caller's acting tenant. So a host supervisor with an enabled activation can:
- start runs on rows they cannot see;
- learn that an id they already hold exists (a successful start creates a run; otherwise they get "not found in this organisation").

This behaviour predates BSU#1233 for placements, and BSU#1233 narrows it for the other types. It is not a regression of that PR.

## Fix (needs a rule on who may start runs)

Recommended: the function applies the same visibility predicate as the subject table's SELECT policy, evaluated for the calling user. A caller can then start runs only on rows they can read, and "not visible" and "not found" return the same message. Harness scenarios: a host supervisor on a hidden row (refused, with no existence signal), the same user on a visible row (allowed), and a non-supervisor member (unchanged).

Refs BSU#1233 (review follow-up F2).
