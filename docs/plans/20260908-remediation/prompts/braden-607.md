...
Claude Code Sonnet 5 is an authorized bounded implementation/review worker: preflight the actual model ID and flags. Codex CLI Astra is the requested escalation for architectural decisions, two failed grounded fixes or final difficult review. Hermes stays on the user's selected Gemini while productive; agy Gemini is an available preflighted CLI option. Grok 4.6 and Kimi are currently quota-exhausted: do not switch back automatically or substitute an unrelated provider. Keep at most two lanes and one writer per owned checkout. Carry source/HEAD, evidence, failures and acceptance gaps in escalation packets; independently review all external diffs.
