...
# Kick off crm7#2628: fix(ci): make Supabase branch provisioning restart-safe and consumer-safe

Implement and verify https://github.com/GaryOcean428/crm7/issues/2628. This is one bounded issue in the existing canonical BSuite programme, not a launch of the whole backlog and not a replacement backlog. Re-read live issue/PR state and source; if a limb is already resolved, validate and reconcile exact evidence instead of rebuilding. Act after reading; do not stop at a plan or ask again whether to continue.
...
- **test-qa-and-verification** — load `/home/braden/.agents/skills/test-qa-and-verification/SKILL.md` and apply only its relevant scope.
...
1. Provision each head SHA through one short serialized mutation job; consumers only resolve and verify readiness.
2. Detect lost advisory-lock ownership in-band and stop mutation until ownership and readiness are re-established.
3. Prove same-SHA concurrency, newer-SHA isolation, cleanup ownership, exact migration/fixture postconditions, and production refusal.
...
This is the live GitHub issue body captured during the 2026-09-14 records reconciliation. It does not override later operator instructions or current repository rules. Live issue: https://github.com/GaryOcean428/crm7/issues/2628. Captured GitHub updatedAt: 2026-09-11T13:48:32Z.

---

## Incident

PR #2627, commit `0b1fb82e40f190ca9348f4cb6610dc241a68d4e8`, exercised the same disposable branch from Lighthouse run `34604644068` and E2E run `34604644126`. No production target was involved.

Sanitized hosted timestamps establish the race:

- Lighthouse acquired the target advisory lock at `13:31:06.694`.
- E2E began waiting at `13:31:15.796` and acquired the same lock at `13:31:25.062`, although Lighthouse did not exit until `13:31:43.950`.
- E2E reset the branch schema from `13:31:25.753` to `13:31:45.213`.
- Lighthouse retried `supabase db push` at `13:31:43.944`, during that reset, and failed at `13:31:43.953` when `20261103000000_workflow_definitions.sql` referenced absent `public.tenants` (`42P01`).
- E2E then completed the build and fixture postcondition and ran `82` tests successfully with `51` skipped.

This proves `42P01` was a concurrent reset observation, not evidence that `workflow_definitions` or the baseline migration is defective. The separate `57P01` in Lighthouse is an infrastructure connection termination; the logs do not identify precisely why the advisory holder stopped.

## Root cause

`crm7/scripts/ci/resolve-supabase-branch.sh` holds a session-level advisory lock through a background `psql` coprocess at lines 391-416, but it only reads the initial acknowledgement at lines 397-404. It does not prove ownership before reset, each push attempt, direct SQL seed, or postcondition. The protected writes use separate connections.

On Bash 5.3, a coprocess that acknowledges and later exits non-zero does not fail the parent under `set -e`; the parent continued its retry path. Bash also unsets `RESET_LOCK_PID` after reaping, so the current EXIT trap can itself fail under `set -u`. PostgreSQL session advisory locks are correctly released at session end, so a target restart can release the lock while its original workflow is still alive.

## Required remediation

1. Make provisioning a short, dedicated job shared by E2E and Lighthouse, with one non-cancelling GitHub job-concurrency group per PR head SHA and a queue that preserves both jobs. Do not put this group around the long E2E or Lighthouse consumer jobs.
2. Give each head SHA its own disposable branch identity. A later push must provision a different target rather than reset the branch that earlier E2E or Lighthouse consumers still use. Both same-SHA workflows resolve the same target.
3. Move all target mutation into the provisioning job: schema reset when needed, migration push, SQL seed, and fixture-account seed. Consumer jobs use a resolve-and-verify-ready path only; they never reset, migrate, or seed. Branch cleanup has a lifecycle owner outside individual consumers, so one successful/failed consumer cannot delete a target another is using.
4. Retain the target-side advisory lock as a second fence for unregistered callers, but add an in-band holder health protocol (a query/response over the same `psql` session, not `kill -0`). On loss, stop mutation, recreate the holder, and re-run the complete readiness decision before any further mutation. Do not continue a push retry from stale ownership. Make teardown tolerate an already reaped coprocess.
5. Treat readiness as an exact postcondition: expected migration version/name set plus seed fixture counts and seeded fixture-account/membership checks. The current line-604 table/ledger counts are useful as a fast hint but are insufficient provenance by themselves. Only a failed readiness check inside the provisioner may reset an unconsumed SHA-scoped target.

## Acceptance evidence

- A local shell harness makes the holder acknowledge then die. The original provisioner performs no later mutation until it has reacquired ownership and revalidated state.
- A two-runner harness reproduces this incident ordering: runner A loses its target holder during a transient push; runner B completes provisioning; runner A then observes ready state and does not reset or push.
- Workflow tests show both `e2e.yml` and `lighthouse-ci.yml` invoke the same short serialized provisioning contract and their consumer jobs use read-only readiness resolution.
- A newer head SHA receives a different disposable branch; it cannot reset an older SHA target while its consumers run.
- A same-SHA E2E/Lighthouse pair produces one ready target, with the second provisioner observing readiness and doing no destructive reset.
- Existing resolver fail-closed production guards remain covered; no production, provider, or live database mutation occurs during local regression tests.

## Scope found

The only direct workflow consumers of the mutating resolver are `crm7/.github/workflows/e2e.yml:88` and `crm7/.github/workflows/lighthouse-ci.yml:95`. The only branch deletion call is `e2e.yml:346`; it must be moved to the new lifecycle owner as part of this work.
