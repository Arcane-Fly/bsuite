...
[`20260810-four-axis-identity-model-and-backlog-sequence-v1.00W.md`](./00-roadmap/20260810-four-axis-identity-model-and-backlog-sequence-v1.00W.md)
...
### V — verification integrity · ~~2 DONE, 3 PARTIAL, 6 OPEN~~ **4 DONE, 4 PARTIAL, 3 OPEN — re-measured 2026-08-18** (+1 class-level finding)

*Added 2026-08-21: **V-0**, class-level. NOT one of the 80 items; does not move the scoreboard — same footing as K-0 and TH-0.*
...
| # | Item | Verdict | Evidence measured 2026-08-17 | Size |
|---|---|---|---|---|
| V-0 | *(class-level, not one of the 80)* A migration RECORDED AS APPLIED that created nothing — and a green applier run that applied nothing | **DONE for the instance found; the CLASS is unguarded** | **Measured on the live database 2026-08-21.** `schema_migrations` holds a row for `20260304090001`. All THREE features it defines are absent: `to_regclass('public.rate_adjustments')` NULL, `to_regclass('public.billing_cycles')` NULL, `gto_complaints.external_referral` column absent. Five months, 742 recorded migrations, and nothing noticed — because **a table that was never created looks exactly like a feature nobody has built yet.** Surfaced only because the R8 lane reported itself blocked on a role gate for rate adjustments. It was not blocked by a missing permissions model; there was nothing to gate. **REPLAYING IT VERBATIM WOULD HAVE SHIPPED FOUR DEFECTS.** `tenant_id UUID NOT NULL REFERENCES auth.users(id)` on BOTH tables — the tenant column pointing at a USER. ONE `FOR ALL USING (...)` policy per table covering select/insert/update/delete with **no role check at all**, so any active member including `guest` could approve a rate adjustment or move a billing cycle to `paid` — and `billing_cycles` carries `gross_amount`, `gst_amount`, `total_amount`. `tenant_id = auth.uid()` repeating the conflation. And a reference to `public.host_employers`, which does not exist. **THE ESTATE'S GATES THEN CAUGHT FIVE MORE IN THE CORRECTION**, every one a defect the author wrote or copied forward: `SET search_path = public` missing `pg_temp`; `award_code TEXT` where the DRY rule requires an FK (`public.awards` has 156 live rows); and **all eight new policies defaulting to `TO PUBLIC`** because `CREATE POLICY` without a `TO` clause does that, plus `REVOKE ... FROM anon` failing to close a `SECURITY DEFINER` grant held through `PUBLIC`. A migration written to correct four defects needed five rounds of correction itself. **A SECOND, SEPARATE FINDING IN THE SAME PASS: the push-triggered applier reports SUCCESS having applied ZERO.** Measured twice — `Detect changed migrations: success` / `Apply migrations: SKIPPED`, which is the exact failure the workflow's own comment documents. Only `gh workflow run supabase-migrate.yml --ref main -f submodule=all` applies anything. The full chain for ONE migration is: merge to submodule development → promote submodule to its main → bump the PARENT gitlink → promote the PARENT to main → DISPATCH → read the database. Five PRs moved `schema_migrations` 742 → 744. **AND THE 81 PENDING MIGRATIONS ARE NOT FAILURES.** `MIGRATION_FLOOR: 20260611000000` — all 81 (69 BSU, 12 conduit) predate it, so **zero are genuinely unapplied**. The applier is bounded, not broken. Partition pending migrations by the floor before raising them; the question is never how many are pending. **FIXED:** crm7 `20260831000000` creates both tables with the tenant FK corrected, four per-operation policies each, requesting separated from approving (the table carries `requested_by` AND `approved_by`), money writes owner/admin only, FK indexes per pgTAP A1, and `anon`+`PUBLIC` revoked. Verified APPLIED on production, not merged: 744 recorded, both tables present, 4+4 policies, 8 of 8 scoped `TO authenticated`, `has_tenant_role()` created. **WHAT IS NOT DONE:** nothing detects the CLASS. No gate compares `schema_migrations` against `to_regclass` for the objects a migration claims to create, so the next phantom is found the same way this one was — by someone tripping over the hole months later. | L |
...
### TH — theme and visual · 1 DONE, 3 PARTIAL, 6 OPEN, 1 SUPERSEDED (+1 class-level finding)

*Added 2026-08-21: **TH-0**, a class-level finding. It is NOT one of the 80 items and does not move the scoreboard — same footing as K-0.*

| # | Item | Verdict | Evidence measured 2026-08-17 | Size |
|---|---|---|---|---|
| TH-0 | *(class-level, not one of the 80)* A form's validation knows which fields are compulsory; the page does not say so | **PARTIAL — CRM7's 39 closed, 13 more found by fixing the gate's own blindness** | **Measured on production 2026-08-21, CRM7 `/clients/create`** — the surface the operator reported as *"an absolute mess"*: **15 form controls, ZERO required markers anywhere on the page**, while the form's own validation read `name: z.string().min(1, 'Business name is required')`. The only way to discover a field was compulsory was to fill the form, press the button, and be rejected. No `aria-required` either, so a screen reader was never told at all. **Nothing errors** — the form works, typecheck passes, the schema is right. The defect lives entirely in what the page does not say, which is why no build gate and no test saw it. **Sweep 1 (crm7#1888, merged):** 39 fields across 17 pages, fixed to CRM7's existing convention — `<span className="text-error-text" aria-hidden="true">*</span>` plus `aria-required` on the control, so a reader announces "Business Name, required" rather than "Business Name star". The convention already existed in 10 components and had a test asserting it on the People form; these pages had simply never adopted it. **THE TEST IS CONSISTENCY, NOT ASTERISK COUNT.** The estate uses **two** valid conventions: CRM7 marks the required fields; **BSU marks the OPTIONAL ones** (`Industry (optional)`). BSU has 95 labelled fields and not one asterisk and is **correct** — `Organization/CreateOrganization.tsx` is 5-for-5 against its schema. A gate that counted asterisks would have condemned a correct form and sent someone to "fix" it. **The gate shipped with two bugs of its own, and both hid real defects.** (1) The attribute scan window was bounded by the next `>` — and `onChange={(e) => ...}` contains one, so the window closed mid-tag and attributes plainly present read as missing. (2) The zod field parser expected `z.` on the same line as the field name, so a field written across lines (`code: z` / `.string()` / `.min(2, ...)`) was reported as **absent from the schema** rather than **present and unmarked** — a silent false negative. `qcr-code`, labelled "Provider code *" with neither `aria-required` nor native `required`, passed a clean scan because of it. Both are now covered: **11/11 self-tests**, including one asserting BSU's inverse convention reads CLEAN and one for each parser bug. Fixing them raised schema-matched fields from 143→161 (crm7) and 9→16 (BSU) and exposed **13 marker gaps + 16 aria gaps** the first run had called clean. **Also corrected:** native `required` satisfies the screen-reader half by itself — it maps to required in the accessibility tree — so demanding `aria-required` as well was a false positive against already-correct controls. **Worse than a bare omission:** three CRM7 compliance forms (guardian consents, WHS audits, LLN assessments) mark SOME required fields and leave others bare *on the same form*. An unmarked field beside marked ones actively reads as optional. **NOT MEASURED, and not counted as passing: 733 labels in crm7 and 75 in BSU** whose schema lives in another file. The gate prints that count on every run rather than reporting a clean zero. Gate: `scripts/check-required-field-markers.mjs --self-test`. | S+M |
...
### PO — portals · 2 DONE, 1 PARTIAL, 2 OPEN — PO-1 closed 2026-08-20, PO-2/4/5 unchanged this pass

| # | Item | Verdict | Evidence measured 2026-08-17 | Size |
|---|---|---|---|---|
| PO-1 | Walled field-officer portal not retired | ~~**OPEN**~~ ~~**PARTIAL — re-measured 2026-08-18**~~ **DONE — re-measured live 2026-08-20, the 2026-08-18 row was chasing the wrong pages** | **The 2026-08-18 pass named the wrong "retirement half."** D-93's own text (`docs/20260814-portals-operator-rulings-v1.00A.md`): *"Do not build portal-shaped duplicates of case notes, site visits, WHS logging, competency or LLN capture; the surfaces under `/field-officers/*` are the ones."* Task 3.1 of the remediation plan repeats it verbatim. `crm7/src/pages/field-officers/{site-assessment,site-visits,case-notes,...}` were never the walled duplicate — they are the ruling's *named destination*, and the field-officer dashboard (`/portal/field-officer`) already navigates into them (`case-notes/create` from its Quick Actions, confirmed reading the live component). The thing D-93 actually orders retired is the WALLED FRAMING on `/portal/field-officer` (and conduit's equivalent) — and that shipped: crm7 `5489361d` / PR #1804 ("retire the walled field-officer portal, fix its broken caseload query") and conduit `00326c8` / PR #485 ("move FO Assignments out of the walled public-portal shell") are both merged to `development`. Verified independently, not taken on the PR title: `PortalScopeGate` (the OAuth-scope wall) no longer wraps `src/pages/portal/field-officer.tsx` — grep for it across `src/pages/portal/*.tsx` now returns only `host-employer.tsx`, `worker-portal.tsx`, `onboarding.tsx`; `src/pages/portal/index.tsx` auto-routes `portalRole === 'field_officer'` straight to the dashboard rather than offering it as a picker choice; conduit's route lives at `src/app/(dashboard)/field-officer-assignments`, not `(portal)/...`. **The mandatory half is also live, not merely authored:** `20260823010000_field_officer_caseload_rls.sql` is applied — `pg_policy` on the live database shows `site_visits_field_officer_caseload_only`, `case_notes_field_officer_caseload_only`, `incidents_field_officer_caseload_only`, `lln_assessments_field_officer_caseload_only` and `competency_assessments_field_officer_caseload_only`, all scoped off `assigned_field_officer_id`. One pre-existing nit, not a blocker (already on record in `docs/20260817-po-portal-verification-v1.00W.md` §2): crm7's URL keeps the `/portal/` segment for the now-unwalled dashboard — cosmetic, since crm7 has no `PortalLayout`-equivalent and every route renders through the same `AppShell`/`ProtectedRoute`. | — |
...
**Portals — 5 items.** PO-4 is a large build. **PO-1 closed 2026-08-20** — the walled-portal
retirement and its caseload RLS are both live; see its row above. PO-2's remaining half is
narrower than filed: identity and the foreign keys exist; only the scoping policy is missing.
