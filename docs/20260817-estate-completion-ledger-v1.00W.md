...
|---|---:|---|
| **DONE** | ~~16~~ ~~19~~ **21** | Re-measured fixed, with evidence. No work remains. |
...
| **PARTIAL** | ~~13~~ ~~14~~ ~~17~~ **15** | Half shipped. Real work remains — counted as open below. |
| **OPEN** | ~~54~~ ~~50~~ **47** | Untouched, or the fix exists but has not reached the running system. |
| **Total** | **87** | |

**Closed: ~~20~~ ~~23~~ 25. Carrying work: ~~67~~ ~~64~~ 62** (~~14 partial + 50 open~~
**15 partial + 47 open**) — roughly **31 small, 23 medium, 13 large** (sizes unchanged; neither recount shifts the
...
pointers were advanced onto the development heads that already carried their fixes. The total
carrying-work count then fell to **62** when AD-4 and AD-7 were actually exercised against the
fixture tenant later the same day — `count(user_id)` on `people` 0→1, `people_portal_invites`
0→1 and accepted 0→1, `data_import_jobs`/`custom_pages`/`form_layouts`/`tenant_entity_records`
each 0→1, every one with a Tenant-B negative control that was refused 42501. AD-5 and AD-6 stay
PARTIAL, because their last step is an operator one: a confirmed target field in BSU, and a
promotion to parent `main` for the applier to run.
...
### AD — shipped but not reaching the product · 2 DONE, 4 PARTIAL, 3 OPEN, 2 NOT-A-DEFECT
