...
| 7 | Continue bsuite#570 after the adapter/credential/settings/status slices — crm7#988 merged MYOB/Astute export adapters, crm7#989 merged Vault-backed provider credential storage plus Astute DB provider support, crm7#990 surfaced those credentials in Settings, and crm7#991 added ADR-compatible provider-passthrough STP/EOFY status tracking on payroll runs. Remaining work is the EOFY confirmation/action flow against provider APIs; direct ATO lodgement remains blocked by ADR 0004 unless superseded. | CRM7 implementer |
