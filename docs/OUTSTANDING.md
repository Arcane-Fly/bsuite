...
| 2 | [bsuite#345](https://github.com/GaryOcean428/bsuite/pull/345) | Phase 3B: Opt-in `rename_physical_column` + FieldEditDialog confirm | **Request changes** | Native `<dialog>` + `<details>` diverges from shadcn per `AGENTS.md` §5 (and `<details>` hides safety-critical warnings — the disclosure is collapsible so the warning can be missed entirely). See **B hardening follow-ups** in the Context section below for five items downgraded from blocker after verification | Copilot to convert FieldEditDialog to Radix `<Dialog>` (or at minimum replace `<details>` with a persistent `<Alert>` / `role="note"` block that stays visible whenever the physical-rename checkbox is checked). Rebase on development after fix; address the 5 hardening follow-ups in the same PR if scope allows, otherwise split |
...
- **Reviews posted**: detailed review comments on each PR (2026-04-28) — retrieve via `gh pr view 346 --repo GaryOcean428/bsuite --comments`, `gh pr view 345 --repo GaryOcean428/bsuite --comments`, and `gh pr view 339 --repo GaryOcean428/crm7 --comments`. A **severity-recalibration addendum** was posted on #345 ([issuecomment-4349518914](https://github.com/GaryOcean428/bsuite/pull/345#issuecomment-4349518914)) after verifying `tenant_entities` seed contents (business-domain entities only, no structural tables), narrowing the true merge-blocker set to the single shadcn / `<details>` UI-consistency item.
- **Test-infra prereq**: commit `3249f98` landed the vitest + jsdom `.rejects.toThrow` workaround (manual `.catch((e) => e)` + `toBeInstanceOf(Error)` + `.message` property match) directly on `development` earlier in this session. It is what unblocked local CI on all three Phase 3 PRs; the three branches will pick up the conflict-fix on their next rebase. Revert the workaround once `vitest@^3` upgrade ships — tracked in the `packages/schema-builder/docs/testing-notes.md` subsection above.
...
- **B hardening follow-ups (downgraded from blocker after verification 2026-04-28)**: the five items below are real but are defense-in-depth / UX-hardening, not security blockers — the SECURITY DEFINER function's actual attack-surface gates (`auth.uid()` null check, role verification against `user_tenants`, `format('%I')` identifier quoting, pinned `search_path = ''`) are all correct. **Ordered by user-impact severity (UX-correctness first, then backend-hardening).** Track as follow-ups, not merge-blockers. Copilot: ship them if easy in the same PR; do NOT block the primary Radix-Dialog fix waiting on these.
  1. **Reserved-keyword + `pg_*` rejection** (hardening) — regex `^[a-z][a-z0-9_]{0,62}$` accepts `select`, `from`, `user`, `pg_class`, etc. `format('%I')` quotes them safely (no injection), but allowing `"select"` as a column name is a long-term foot-gun for BI tools + ORM scaffolding. Add `IF p_new_name IN (SELECT word FROM pg_get_keywords() WHERE catcode IN ('R','T')) …` + `LIKE 'pg\_%' ESCAPE '\'` rejections.
  2. **Protected-table blocklist** (hardening) — verified 2026-04-28: `tenant_entities` is seeded with business-domain entities only (`contact`, `lead`, `apprentice`, `opportunity`, `employer`, `charge_rate`, `timesheet`, `host_employer_agreement`, etc.) — NOT structural tables like `tenants`, `user_tenants`, `schema_mutations_audit`, `payment_methods`, `platform_branding`. So the attack surface is currently empty, BUT there is no upstream constraint preventing a future migration from seeding a structural-table row; add a belt-and-braces `IF v_table_name IN (…structural tables…) THEN RAISE …` guard as defense-in-depth.
  3. **`affected_views` / `affected_policies` precision** (UX-correctness) — dry-run uses `ILIKE '%' || v_old_name || '%'` which false-positives every view/policy containing the word in any context (renaming a column called `id` would list virtually every view). Users will learn to ignore the list, defeating the purpose of the confirmation UX. Switch to word-boundary regex (`~* ('\m' || v_old_name || '\M')`) or traverse `pg_depend` for actual column references.
  4. **Stale dry-run preview in FieldEditDialog** (UX-correctness — most user-impactful item) — after the user triggers a dry-run, they can still edit the name input BEFORE clicking confirm. The DDL shown in the confirmation modal then becomes a lie. Either (a) invalidate `confirmState` → `idle` on any form-field change after a preview is generated and show an inline "preview out of date — click Preview again" hint, or (b) re-run dry-run inside the confirm handler and refuse to proceed if the generated SQL differs. Ships independently of the primary Radix-Dialog primitive decision, so keep this fix out of the shadcn-migration dependency chain.
  5. **TOCTOU hash between dry-run and wet-run** (backend-hardening) — dry-run enumerates at time T, user confirms at T+30s, wet-run may hit a newer view/policy added in between. Close by hashing the dry-run preview and accepting `p_preview_hash text` as a required arg on wet-run that must match, or by having wet-run re-enumerate and fail-closed on divergence.
