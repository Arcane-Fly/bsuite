...
| 2 | [bsuite#345](https://github.com/GaryOcean428/bsuite/pull/345) | Phase 3B: Opt-in `rename_physical_column` + FieldEditDialog confirm | **Request changes** | Native `<dialog>` + `<details>` diverges from shadcn per `AGENTS.md` §5 (and `<details>` hides safety-critical warnings); stale-preview correctness bug in FieldEditDialog | Copilot to convert FieldEditDialog to Radix `<Dialog>` (or at minimum replace `<details>` with a persistent `<Alert>`/`role="note"` block that stays visible whenever the physical-rename checkbox is checked) and invalidate the dry-run preview on any form-field change after preview is generated. See **B hardening follow-ups** bullet in the Context section below for items downgraded from blocker after verification. Rebase on development after fixes |
...
- **B hardening follow-ups (downgraded from blocker after verification 2026-04-28)**: the four items below are real but are defense-in-depth / UX-hardening, not security blockers — the SECURITY DEFINER function's actual attack-surface gates (`auth.uid()` null check, role verification against `user_tenants`, `format('%I')` identifier quoting, pinned `search_path = ''`) are all correct. Track these as follow-ups, not merge-blockers:
  1. **Reserved-keyword + `pg_*` rejection** — regex `^[a-z][a-z0-9_]{0,62}$` accepts `select`, `from`, `user`, `pg_class`, etc. `format('%I')` quotes them safely (no injection), but allowing `"select"` as a column name is a long-term foot-gun for BI tools + ORM scaffolding. Add `IF p_new_name IN (SELECT word FROM pg_get_keywords() WHERE catcode IN ('R','T')) …` + `LIKE 'pg\_%' ESCAPE '\'` rejections.
  2. **Protected-table blocklist** — verified 2026-04-28: `tenant_entities` is seeded with business-domain entities only (`contact`, `lead`, `apprentice`, `opportunity`, `employer`, `charge_rate`, `timesheet`, `host_employer_agreement`, etc.) — NOT structural tables like `tenants`, `user_tenants`, `schema_mutations_audit`, `payment_methods`, `platform_branding`. So the attack surface is currently empty, BUT there is no upstream constraint preventing a future migration from seeding a structural-table row; add a belt-and-braces `IF v_table_name IN (…structural tables…) THEN RAISE …` guard as defense-in-depth.
  3. **`affected_views` / `affected_policies` precision** — dry-run uses `ILIKE '%' || v_old_name || '%'` which false-positives every view/policy containing the word in any context (renaming a column called `id` would list virtually every view). Users will learn to ignore the list, defeating the purpose of the confirmation UX. Switch to word-boundary regex (`~* ('\m' || v_old_name || '\M')`) or traverse `pg_depend` for actual column references.
  4. **TOCTOU hash between dry-run and wet-run** — dry-run enumerates at time T, user confirms at T+30s, wet-run may hit a newer view/policy added in between. Close by hashing the dry-run preview and accepting `p_preview_hash text` as a required arg on wet-run that must match, or by having wet-run re-enumerate and fail-closed on divergence.
