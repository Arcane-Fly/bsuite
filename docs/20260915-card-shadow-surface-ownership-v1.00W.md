...
Canary 2.7.1-next.4 keeps an explicit padding choice on the painted surface. A chrome-owning grid item applies it directly; a layout-only wrapper exposes `data-card-padding` and the existing `--card-padding` value for consumer Cards. A consumer can use the scoped Tailwind variant `[[data-card-padding]_&]:p-(--card-padding)` without applying an app's global card-padding token to untouched cards. This pairing must ship together; the marker alone does not update a consumer's Card.
