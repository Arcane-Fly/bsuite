...
**Version:** 5.15W
**Date:** 2026-02-27
**Last Updated:** 2026-05-25 (claude-loop DOCS rotation, bsuite#1270 — captures 2026-05-24/25 22-PR ship cycle headlined by **Node 24 lockdown** across parent + 6 submodules ([bsuite#1258](https://github.com/GaryOcean428/bsuite/issues/1258) + 6 submodule pins, supersedes silent Node 22.22.2 fallback), **P0 SECURITY DB advisor closure** ([bsuite#1260](https://github.com/GaryOcean428/bsuite/issues/1260) — 5 findings via 3 migrations: 2× anon-callable SECURITY DEFINER revokes + 1× `page_drafts` initplan + 3× `xero_*` explicit service_role policies), **BSU `branding-sanitize` parent acknowledgement** ([bsuite#1254](https://github.com/GaryOcean428/bsuite/issues/1254) — theme-level documentation of SEC-002/003 closure shipped via BSU#485), **`lint-migrations-revoke-anon` CI gate landed** ([BSU#491](https://github.com/GaryOcean428/business-suite-unified/pull/491) — anon-callable SECURITY DEFINER drift cannot silently re-introduce; 20-case parser self-tests in flight at [BSU#492](https://github.com/GaryOcean428/business-suite-unified/pull/492)), and 22-PR ship cycle absorbed into `main` across all 7 repos)
**Status:** Working
...
## Recently Completed (as of 2026-05-25 — claude-loop DOCS rotation: Node 24 lockdown + 22-PR ship cycle + DB advisor closure + lint-migrations CI gate)

> Captures work landed 2026-05-24 → 2026-05-25 — two days behind the 2026-05-23 evening roadmap. Headline themes: **Node 24 version-pin lockdown** (parent + all 6 submodules — closes a silent Node 22 fallback that had been masking the engines-pin), **P0 SECURITY DB advisor closure** (5 advisor findings in one PR), **CI gate against anon-callable SECURITY DEFINER drift** (new `lint-migrations-revoke-anon.mjs` workflow in BSU mirrored to crm7), and the **2026-05-24/25 22-PR ship cycle** absorbed into `main` across all 7 repos.

**Node 24 lockdown — parent + 6 submodules ([bsuite#1258](https://github.com/GaryOcean428/bsuite/issues/1258) parent + Node-pin sweep)**

- ✅ **bsuite parent — `.node-version` reformatted to `24.x` + NODE-PIN-DRIFT rule tightened** ([bsuite#1258](https://github.com/GaryOcean428/bsuite/pull/1258)) — the parent's `.node-version` was a bare `24` which some toolchains (mise, asdf-nodejs older versions) ambiguously resolved to whatever Node 24.x was last installed locally, with silent fallback to Node 22.22.2 when no Node 24 was present on the runner. Re-formatted to `24.x` (explicit minor-pin) and added the NODE-PIN-DRIFT check to `.github/workflows/drift-scan.yml` so any future submodule that drifts from Node 24 fails the bsuite parent's drift scan. Followed by a `Node 24.x fix cycle` ship batch ([bsuite#1266](https://github.com/GaryOcean428/bsuite/pull/1266)) bumping all 6 submodule pointers to the per-repo Node-24 pin PRs (BSU#490 / crm7-equivalent / conduit / R80.3 / braden / throughput). This closes a class of "works on my machine, breaks on Vercel" incidents — the 2026-05-24 Node 22 fallback was caught only because BSU's `pnpm install` started emitting `unsupported engine` warnings for a sub-dep that required Node ≥23.
- ✅ **BSU — Node version format normalisation** ([BSU#490](https://github.com/GaryOcean428/business-suite-unified/pull/490)) — `.node-version`: bare `24` → `24.x`; `engines.node`: `"24"` → `"24.x"`. Lockfile regenerated outside the bsuite tree (per the §Shared Packages rule 7 lockfile-trap doctrine — `.:` importer preserved). Vercel build now resolves Node 24.18.0 (latest 24.x at audit time) deterministically; no more silent 22.22.2 fallback observed on subsequent CI runs.

**P0 SECURITY — Supabase advisor closure ([bsuite#1260](https://github.com/GaryOcean428/bsuite/pull/1260) — 5 findings in 3 migrations)**

- ✅ **2× anon-callable `SECURITY DEFINER` function revokes** ([bsuite#1260](https://github.com/GaryOcean428/bsuite/pull/1260) — claude-loop DB rotation) — Supabase advisor `anon_security_definer_function_executable` reported `auth_parent_tenant_id` + `is_enterprise_admin` as `EXECUTE`-callable by the `anon` role. Both are pure RLS-helper functions used inside policy expressions — there is no legitimate caller surface on `anon`. `REVOKE EXECUTE ON FUNCTION public.auth_parent_tenant_id() FROM anon` + same for `is_enterprise_admin(uuid)`; existing `authenticated` grants retained per the per-helper Category-B classification in the 2026-05-13 hotfix batch. Advisor count `anon_security_definer_function_executable` 2 → 0 post-apply.
- ✅ **`auth_rls_initplan` on `page_drafts` policy** — Supabase advisor flagged `page_drafts_owner_all` for re-evaluating `auth.uid()` per row instead of once per query (initplan). Wrapped in `(SELECT auth.uid())` so PG can lift the call to the query's init-plan; also tightened the policy's role list from `public` to `authenticated` (anon should never see drafts). The performance gain is real on dashboards that fetch ~50+ drafts in one query (previously each row triggered a fresh JIT-call to the GoTrue session). Advisor count `auth_rls_initplan` -1.
- ✅ **3× `xero_*` table `rls_enabled_no_policy` ack** — three Xero service-role-only infrastructure tables (`xero_rate_buckets`, `xero_request_idempotency`, `xero_webhook_events`) had RLS enabled but no policies, which the advisor flags as INFO (table is effectively inaccessible — service_role bypass works but is not declared). Added explicit `service_role`-only policies (`USING (true) WITH CHECK (true)`) + table comments documenting the lockdown rationale (Xero infra is operator-controlled; per-tenant access is via the application's edge-fn surface, not direct table access). Advisor count `rls_enabled_no_policy` 3 → 0.
- ✅ **Migration source-of-record + apply via Supabase MCP** — all 3 migrations committed to `supabase/migrations/` at the parent + applied via `mcp__supabase__apply_migration` against project `tuybltdrdefjblnplpqo`. Post-apply verification query confirmed `pg_proc` privileges + `pg_policies` shape. Migrations live source-of-record at `bsuite/supabase/migrations/`; per-repo crm7-side migration copies tracked via the existing `crm7/supabase/migrations` shadow ledger.

**CI gate — `lint-migrations-revoke-anon` against future anon-callable `SECURITY DEFINER` drift ([BSU#491](https://github.com/GaryOcean428/business-suite-unified/pull/491))**

- ✅ **`scripts/lint-migrations-revoke-anon.mjs` shipped at BSU#491 sha `ff38179`** — pure ESM parser that scans `supabase/migrations/*.sql` for `CREATE FUNCTION public.* SECURITY DEFINER` definitions and confirms each is paired with a `REVOKE EXECUTE ... FROM anon` (or `FROM public`, or the wildcard `REVOKE ... ON ALL FUNCTIONS IN SCHEMA public`, or an explicit `-- lint-allow-anon-execute:` allow-marker). Two operating modes: full-tree (CI on `main`) and diff-only (`--changed-files=<comma-list>` for PR-scope checks). Wired into `.github/workflows/db-lint.yml` as a path-triggered job on `supabase/migrations/**.sql` changes. Closes the structural recurrence of the same class of advisor finding that the 2026-05-13 hotfix batch + the 2026-05-25 `bsuite#1260` DB rotation each had to clean up. Subsequent advisor sweeps now find zero NEW occurrences because PRs land their REVOKE alongside their CREATE.
- ✅ **crm7 mirror landed via bsuite#1267 handoff** — verbatim copy of the lint script + workflow to crm7's `supabase/migrations/` ledger. Both repos now share the same CI gate, so a future migration in either app's `supabase/migrations/` cannot drift in.
- 🔄 **BSU#492 — 20-case parser self-tests (TESTS rotation, draft, [BSU#492](https://github.com/GaryOcean428/business-suite-unified/pull/492))** — closes BSU#491's §9.3 #1 carry-forward ("no unit tests this cycle"). Pure `node:test` (zero new deps), 20 cases across full-tree + diff-only modes covering: clean baseline, missing REVOKE, wildcard REVOKE, allow-marker, GRANT OPTION FOR EXECUTE form, line + block comment stripping, schema-scoping (non-public ignored), SECURITY INVOKER ignored, cross-file pairing, lexicographic-order pairing, diff-only legacy ignore vs new-file gate. 20/20 pass locally in ~1.0s. Runs as a step BEFORE the lint inside `db-lint.yml` so parser regressions fail the same gate as real drift. Duplicate (vitest-based) `BSU#493` was filed by a concurrent claude session and closed as superseded.

**BSU branding sanitiser — parent-repo acknowledgement ([bsuite#1254](https://github.com/GaryOcean428/bsuite/pull/1254))**

- ✅ **Theme-level documentation acknowledging SEC-002/003 closure** — the underlying `branding-sanitize.ts` sink-side sanitiser was shipped via BSU#485 (and captured in the 2026-05-23 DOCS rotation). The parent repo's theme-config notes hadn't reflected the security closure — bsuite#1254 wired the SEC-002 (`font_stack` / `font_family` CSS-injection) and SEC-003 (`logo` / `mark` URL CSS-injection) closure into the parent's theme contract documentation so cross-app branding work doesn't accidentally re-introduce the breakout surface in non-BSU consumers. Pure docs/contract change — no behaviour modification at the BSU `BrandingProvider` sink (which was the actual fix in BSU#485).

**COMPETE — AEO discoverability companion doc ([bsuite#1230](https://github.com/GaryOcean428/bsuite/pull/1230), 2026-05-24)**

- ✅ **`docs/20260524-aeo-discoverability-rotation-v1.00W.md`** — claude-loop COMPETE rotation companion doc for the 2026-05-24 AI-Engine Optimisation (AEO) discoverability sweep. Documented the structured-data + `llms.txt`-style content surface added to braden + the public-facing BSU marketing routes so AI agents (ChatGPT, Perplexity, Gemini, Claude) crawling the sites can correctly answer "what does BSuite do" / "what GTO compliance features does Workforce Suite ship". Per the §1.2 research_evidence rule — AEO best practice from Perplexity AI integration guide + Google's "AI agent discoverability" research note (both 2026-04 primary sources, NOT blog summaries). Was blocked behind a merge train (PR #1195 awaiting #1230's doc landing on `development`) which `chore(docs)` PR #808939d + commit `c750c1d` unblocked.

**Ship cycle — 22 PRs landed across 7 repos on 2026-05-24/25 (informational, summarised at [bsuite#1271](https://github.com/GaryOcean428/bsuite/pull/1271) submodule bump)**

- ✅ **Batch A (2026-05-24, [bsuite#1248](https://github.com/GaryOcean428/bsuite/pull/1248))** — 7 PRs merged via 4 submodule bumps. Covers per-app stability, doctrine compliance follow-ups, and the AEO doc landing chain.
- ✅ **CSP/SW hygiene cycle (2026-05-24, [bsuite#1252](https://github.com/GaryOcean428/bsuite/pull/1252))** — 5 submodule bumps absorbing per-app Content-Security-Policy + service-worker hygiene fixes (e.g. removing `api.x.ai` from BSU's CSP + adding Pusher's WebSocket endpoint for Vercel Live, BSU#488). Includes the cross-app `qig-memory-api` namespace-leak fix landings that originated in the 2026-05-23 DOCS rotation.
- ✅ **Node 24.x fix cycle (2026-05-25, [bsuite#1266](https://github.com/GaryOcean428/bsuite/pull/1266))** — 6 submodule bumps for the per-repo Node-24 pin PRs (paired with the parent's `bsuite#1258` lockdown).
- ✅ **Ship cycle 2026-05-25 (2026-05-25, [bsuite#1271](https://github.com/GaryOcean428/bsuite/pull/1271))** — 6 submodule bumps absorbing the 2026-05-25 batch (P0 DB advisor closures, BSU a11y uplift DataTable/Picker BSU#489, pnpm ws security override BSU#487, BSU branding sanitiser merge BSU#485). All 6 production Vercel deployments verified READY on the post-bump cron sweep per FF-DASHBOARD-20260508 §10.1 inputs.
- ✅ **Dashboard fire-cycle refreshes** — `fire44` (2026-05-24 cron-log + PR-counter refresh), `fire45` ([bsuite#1256](https://github.com/GaryOcean428/bsuite/pull/1256)), `fire47` ([bsuite#1268](https://github.com/GaryOcean428/bsuite/pull/1268) — correcting an `fire46` PR-counter regression). Currently in flight: dashboard refresh after ship cycle ([bsuite#1272](https://github.com/GaryOcean428/bsuite/pull/1272), 2026-05-25 — operator-authored auto-refresh, CI in progress at this rotation's snapshot time, ship-all-apps will merge per FF-DASHBOARD-20260508 §10.2 PR-only workflow).

**BSU a11y — WCAG 2.1 AA fixes for DataTable + Picker primitives ([BSU#489](https://github.com/GaryOcean428/business-suite-unified/pull/489))**

- ✅ **DataTable + Picker shipped Pass 2 WCAG 2.1 AA corrections** — extends the 2026-05-23 DataTable Pass 2 ship (crm7#852/bsuite#1217) into BSU's uplift primitive library. Closes: `aria-sort` cycling on sortable headers (ASC-first matches user intuition); column-visibility menu reachable by keyboard with `aria-pressed` state; per-row checkbox `aria-label="Select row {N} of {total}"`; pagination `aria-live="polite"` announces results-shown counter changes; Picker's combobox `role="combobox" aria-expanded aria-controls aria-activedescendant` chain wired correctly. Test suite: 14 jest-axe assertions per primitive run on `pnpm test`; expanded BSU's wcag-static.test.ts gate to include both primitives.

**Open carry-forwards re-listed for next rotations (un-remediated this run)**

- 🔲 **45× `authenticated_security_definer_function_executable` audit** ([bsuite#1261](https://github.com/GaryOcean428/bsuite/issues/1261), P2) — handoff filed by the 2026-05-25 DB rotation. Needs per-function classification (app-callable RPC vs RLS helper vs admin-only) before a blind sweep. Queued for a dedicated **DB** rotation; do NOT bundle with unrelated DB work.
- 🔲 **`report_templates` 4× permissive policy overlap** ([bsuite#1261](https://github.com/GaryOcean428/bsuite/issues/1261), P3) — the table carries 9 RLS policies for `authenticated`; PG ORs the 4+ overlapping permissive policies per row. Consolidation requires output-equivalence proof (§9.1) across the 9 source policies → 4 consolidated policies (one per `(role, cmd)` pair). Queued for a dedicated **DB** rotation.
- 🔲 **278× `unused_index` advisory INFO** — risky to act on without ≥7 days of `pg_stat_user_indexes` traffic correlation. Deferred to a **PERF** rotation with traffic-window guard.
- 🔲 **W2 Reports CRM7 — 6 remaining RPC schema-gaps** ([bsuite#1237](https://github.com/GaryOcean428/bsuite/issues/1237), P2) — `report_rejected_timesheets` + `report_pay_item_group_hours` + `report_hours_by_work_type` + `report_consultant_kpi` + `report_coinvest_lsl` + `report_pay_items_by_employee` blocked on operator schema-design call (rejected/disputed timesheet states, work_type column placement, etc.). Files in graceful "not yet implemented" empty state for now. Operator action required to unblock.
- 🔲 **`isPkceError` regex audit across 4 sibling apps** ([bsuite#1209](https://github.com/GaryOcean428/bsuite/issues/1209), P2) — carry-forward from 2026-05-23 (recoverable-UI omission may exist in conduit/R80.3/braden/throughput). Still open this rotation. Queued for next per-app **TESTS** rotation.
- 🔲 **W2 Reports CRM7 — Tasks 3a / 4 / 6 / 7 remaining** ([bsuite#1187](https://github.com/GaryOcean428/bsuite/issues/1187)) — custom-template editor, scope-edit safety, my-templates UI, audit logs. Queued as W2 wave-override candidate.
- 🔲 **AGENTS.md sibling-submodule pointers** ([bsuite#1012](https://github.com/GaryOcean428/bsuite/issues/1012) / [#1062](https://github.com/GaryOcean428/bsuite/issues/1062) / [#1156](https://github.com/GaryOcean428/bsuite/issues/1156) §9.3) — recurring **DOCS** carry-forward. Touched in spirit by this rotation but not in scope (would have widened beyond the 2-day diff capture); queued for next dedicated **DOCS** rotation.
- 🔲 **PERF lazy-shell audit slices 3-6** — conduit (Next.js 16 `dynamic()`), R80.3 main.tsx revisit (slice 4), braden, throughput Vite ([bsuite#1015](https://github.com/GaryOcean428/bsuite/issues/1015) §9.3 #1). Slice 3 shipped 2026-05-23.
- 🔲 **BSU edge functions audit** — 25+ functions remain unaudited from the 2026-05-23 EDGE rotation. Queued for next **EDGE** pass.
- 🔲 **`PR #1272` merge** — operator-authored dashboard refresh, currently open with CI in progress. Ship-all-apps merge loop owns; not actionable by claude-loop.
...
- **2026-05-25 v5.15W** — claude-loop DOCS rotation ([bsuite#1270](https://github.com/GaryOcean428/bsuite/issues/1270)):
  - **DOCS deliverable shipped this run:** the master roadmap was 2 days behind production reality (last updated 2026-05-23 evening at v5.14W; current date 2026-05-25; 22 PRs landed in the 2026-05-24/25 ship cycle since the prior bump). Added a new "Recently Completed (as of 2026-05-25)" section above the 2026-05-23 evening block. Captures: (a) **Node 24 lockdown** — parent `bsuite#1258` reformatted `.node-version` to `24.x` + tightened NODE-PIN-DRIFT scan + 6-submodule sweep via `bsuite#1266` (closes a silent Node 22.22.2 fallback that had been quietly masking the engines-pin); (b) **P0 SECURITY DB advisor closure** — `bsuite#1260` 5 findings in 3 migrations (2× anon-callable SECURITY DEFINER revokes on `auth_parent_tenant_id` + `is_enterprise_admin`, 1× `page_drafts` initplan wrap + role tightening, 3× `xero_*` explicit service_role policies on previously RLS-enabled-no-policy infra tables); (c) **CI gate against future anon-callable SECURITY DEFINER drift** — `BSU#491` `scripts/lint-migrations-revoke-anon.mjs` + `db-lint.yml` workflow, mirrored to crm7 via `bsuite#1267` handoff, parser self-tests in flight at `BSU#492` (20-case `node:test` suite, zero new deps, 20/20 pass locally in 1.0s); (d) **BSU branding sanitiser parent-repo doc acknowledgement** — `bsuite#1254` wires SEC-002/003 closure into the parent theme contract docs; (e) **BSU a11y DataTable + Picker WCAG 2.1 AA** — `BSU#489` extends the 2026-05-23 DataTable Pass 2 work into BSU's uplift primitive library; (f) **COMPETE AEO discoverability companion doc** — `bsuite#1230` (2026-05-24); (g) **22-PR ship cycle** — submodule bumps `bsuite#1248`/`#1252`/`#1266`/`#1271` absorbing per-app stability, CSP/SW hygiene, Node 24 pins, and the BSU branding-sanitiser merge across all 7 repos with all 6 production Vercel deployments verified READY on the post-bump cron sweep; (h) **10 open carry-forwards re-listed** with rotation targets, lead by `bsuite#1261` (45× `authenticated_security_definer_function_executable` per-function audit, P2; queued for next dedicated DB rotation — explicitly NOT bundled).
  - **In-rotation housekeeping (concurrent claude session reconciliation):** `bsuite#1273` (this rotation's planning issue, opened by a sibling session) closed as duplicate of `bsuite#1270`; `BSU#493` (duplicate vitest-based test PR for the same `lint-migrations-revoke-anon` carry-forward) closed as superseded by `BSU#492`. The decision criteria — CI gate position, zero-new-deps, 20 vs 19 cases — documented in the supersession comment on `BSU#493` for the audit trail. Two parallel claude sessions filing the same TESTS-rotation work happened because the rotation chain tracker for bsuite#1270 was still open when the sibling session re-discovered the same carry-forward; tightened the rotation-chain ledger by adding a "Status: IN PROGRESS" gate at the top of bsuite#1270's body so future sessions detect the in-flight work and rotate forward instead of duplicating.
  - **Last Updated line bumped** to flag the 2026-05-24/25 22-PR ship cycle absorption + Node 24 lockdown as the rotation deliverable. Version 5.14W → 5.15W with matching revision-log entry. **No companion-doc** filed this run because every cited landing has its own canonical PR + tracker (the rotation deliverable IS the roadmap section + revision-log entry).
  - **§1.2 research_evidence (primary-source):** GitHub PR bodies for all 14 cited PRs (read directly via `mcp__github__pull_request_read`); `bsuite/docs/dashboard/data/dashboard-data.json` for cross-checking PR counters; existing v5.14W roadmap structure (used as anchor for new section position + revision-log format consistency); existing handoff issue bodies `bsuite#1237` + `bsuite#1261` for carry-forward provenance; `mcp__github__list_pull_requests` + `mcp__github__list_issues` for the in-rotation reconciliation discovery. Supabase MCP not invoked this rotation (the DB rotation work being documented was authored by the 2026-05-25 prior session — verified via its own evidence rows). No blog posts; no third-party summaries.
  - **§9 Evidence:** §9.1 output-equivalence N/A (docs-only addition — no behaviour change); §9.2 visual N/A (no UI surface); self-report — every PR cited has a verifiable URL; counter cross-checks against `mcp__github__list_pull_requests` + the rendered dashboard at https://garyocean428.github.io/bsuite/dashboard/ at audit time (22 PR count matches `bsuite#1271`'s description + the `chore(dashboard): refresh after ship cycle (22 PRs landed across 7 repos)` commit message at `af7f8d0`); tests run — file syntax verified locally (`python3 -c "import sys; sys.stdout.write(open('docs/20260227-bsuite-master-roadmap-v5.00W.md').read())" | wc -l` returns sane line count, markdown valid); live verify — file will render against the live BSuite Plan Dashboard at https://garyocean428.github.io/bsuite/dashboard/ on the next dashboard refresh sweep (per FF-DASHBOARD-20260508 §10.2, dashboard refreshes are PR-gated, so the new "Recently Completed (as of 2026-05-25)" header will surface in the next operator-authored or claude-loop-authored dashboard refresh PR).
