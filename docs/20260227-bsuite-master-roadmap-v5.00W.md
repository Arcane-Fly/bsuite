...
**Version:** 5.14W
**Date:** 2026-02-27
**Last Updated:** 2026-05-23 evening (claude-loop ROADMAP rotation, bsuite#1219 — drained bsuite#1216 W2 Reports migrations to Supabase prod via 3 applied migrations including a `template_key` column-restore pre-migration that surfaced a `reconciled-2026-05-19` schema squash regression — `template_key` was silently dropped from `report_templates` on 2026-05-19, breaking the host-pack edge function + `src/pages/portal/host-reports.tsx` for 5 days; restored idempotently + audit follow-up filed as bsuite#1221; also captures bsuite#1213 PERF R80.3 entrypoint lazy-shell `−176 KB` main chunk + bsuite#1217/crm7#852 DataTable Pass 2 W2-prereq wave-override landed earlier today)
**Status:** Working
