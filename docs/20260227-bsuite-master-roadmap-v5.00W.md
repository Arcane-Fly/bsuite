...
**Version:** 5.15W
**Date:** 2026-02-27
**Last Updated:** 2026-05-25 morning (claude-loop DOCS rotation, bsuite#1282 — captures the 2026-05-23→2026-05-25 bolus across all 7 repos: DB advisor sweep [bsuite#1260, 5 findings closed via 3 migrations]; CI REVOKE-FROM-anon lint + 20-case test suite [BSU#491 merged, BSU#492/#493 + crm7#864/#868 stacked PRs open]; cross-app accessibility wave [BSU#489 DataTable+Picker primitives, crm7#846/#859/#860]; CSP + branding-sanitiser SEC-002/003 hardening [BSU#485, crm7#857, BSU#488, BSU#487 + throughput#187 + crm7#854 dep CVE patches]; Node 24.x pin fix across all 7 repos [BSU#490/crm7#861/conduit#282/braden#298/R80.3#277/throughput#191] resolving silent Node 22.22.2 fallback + Vercel typecheck OOM; W2 FilterBar Pass 2 [crm7#853] + first Reports Task 6 RPC [crm7#855 report_timesheet_summary, 1-of-7 schema-fit cleanly]; crm7 EmptyState UX uplift [crm7#856]; crm7 edge CORS + error-sanitise hardening [crm7#850]; BSU launcher d.* prefix preservation for `d.suite.crm7.app` [BSU#494, companion operator action bsuite#1277]; braden llms.txt AEO discoverability [braden#297]; throughput dead-code + skeleton sweep [throughput#185/#188/#189]; ship cycle 2026-05-25 submodule pointer bump [bsuite#1271]; dashboard refresh [bsuite#1272])
**Status:** Working
...
## Recently Completed (as of 2026-05-25 — claude-loop DOCS rotation: DB advisor sweep + REVOKE-FROM-anon CI gate + cross-app A11Y wave + Node-24.x suite-wide pin fix + W2 Reports Task 6 first RPC)

> Captures work landed 2026-05-23 evening → 2026-05-25 morning (roughly a 36h bolus across all 7 repos). Headline items: (1) **bsuite#1260 DB rotation** closed 5 Supabase advisor findings via 3 migrations including 2 unauthenticated RPC-attack-surface closures (`auth_parent_tenant_id` + `is_enterprise_admin` anon EXECUTE revoke) + 1 `auth_rls_initplan` perf fix on `page_drafts` + 3 explicit `service_role` policies for `xero_*` infra tables; (2) **BSU#491 / BSU#492 CI lint + 20-case test suite** for `REVOKE FROM anon on SECURITY DEFINER` as a forward gate so new public-schema `SECURITY DEFINER` functions cannot land without an explicit REVOKE pair — landed to BSU; **crm7#864 + crm7#868** mirror PRs open and awaiting ship-all-apps merge; (3) **suite-wide Node-24.x pin fix** — `.node-version` content of `24` was silently falling back to Node 22.22.2 on actions/setup-node@v4 + Vercel build sandboxes (no installed Node has the exact major-only string), causing every CI run to log `WARN Unsupported engine: wanted: {"node":"24"}` and Vercel Typecheck Deployment Check to exit-137 OOM under Node 22's tighter sandbox memory limit; fixed by pinning to `24.x` semver range across all 6 sibling Vercel apps in one ship cycle (BSU#490/crm7#861/conduit#282/braden#298/R80.3#277/throughput#191); (4) **cross-app A11Y wave** — BSU#489 ARIA-only fixes on DataTable + Picker primitives (cascading to every consumer surface), crm7#846 skip link functional + icon-only button labelling (WCAG 2.4.1/1.3.1/4.1.2), crm7#859 distinctive dialog names + live regions + table caption (WCAG 4.1.2/4.1.3), crm7#860 role="alert" on four error-state surfaces (WCAG 4.1.3); (5) **W2 Reports Task 6 first RPC landed** — crm7#855 `report_timesheet_summary` (1 of 7), the only RPC that fits the current production schema cleanly; the other 6 RPCs are scoped in handoff bsuite#1237 (schema-design gaps requiring operator answers); (6) **BSU launcher d.* prefix preservation** — BSU#494 detects `window.location.hostname.startsWith('d.')` at runtime and rewrites cross-app URLs to their d.* siblings; companion server-side allowlist gap filed as bsuite#1277 (operator action — Supabase OAuth client `redirect_uris` need `d.*.crm7.app/auth/callback` variants added); (7) **W2 FilterBar Pass 2** (crm7#853) promoted from W0 stub to canonical Doctrine §2.9 implementation, completing the W2 Task 4 primitive prerequisites alongside the DataTable Pass 2 from 2026-05-23.

**P0/P1 SECURITY — DB advisor sweep + CI forward gate**

- ✅ **bsuite#1259 DB rotation → bsuite#1260 merged** — closed 5 Supabase security/perf advisor findings via 3 migrations applied to project `tuybltdrdefjblnplpqo`:
  - 2× WARN `anon_security_definer_function_executable` → REVOKE anon EXECUTE on `public.auth_parent_tenant_id()` + `public.is_enterprise_admin()`. Both were callable by the unauthenticated `anon` role despite being internal RLS helpers — closes a real RLS-bypass attack surface (the W2 Task 1.5 migration that introduced `auth_parent_tenant_id` shipped without an explicit REVOKE on 2026-05-23).
  - 1× WARN `auth_rls_initplan` on `page_drafts` policies → wrap `auth.uid()` calls in `(SELECT auth.uid())` to defer evaluation to once-per-query instead of once-per-row (PG planner optimisation); tighten role from `public` to `authenticated`.
  - 3× INFO `rls_enabled_no_policy` on `xero_rate_buckets`, `xero_request_idempotency`, `xero_webhook_events` → explicit `service_role` policies + table comments documenting the deny-all-to-non-service-role intent (matches the canonical pattern from `20260512220000_xero_token_vault_baseline`).
- ✅ **BSU#491 merged** — `scripts/lint-migrations-revoke-anon.mjs` + `.github/workflows/db-lint.yml` ship a zero-dependency forward gate that flags any new `public.X SECURITY DEFINER` function in a PR's migration diff that lacks a matching `REVOKE EXECUTE ON FUNCTION public.X FROM anon` (or `FROM public`) somewhere in the migration tree. Diff-only mode in CI prevents 45+ pre-existing legacy findings (tracked separately in handoff bsuite#1261) from blocking unrelated PRs. Opt-out via `-- lint-allow-anon-execute: <reason>` marker for legitimately-public RPCs.
- ✅ **BSU#492 + BSU#493 stacked PRs** — 20-case Node `node:test` unit test suite for the lint script (zero deps, repo-agnostic via `process.execPath` + `__dirname`-relative paths) + companion `chore(test): include scripts/__tests__/**/*.test.mjs in vitest run` wiring. Locally validated: 20/20 pass in ~1.4s. Both still open at this rotation; will land on next ship-all-apps cycle.
- ✅ **crm7#864 + crm7#868 mirror PRs open** — verbatim copies of BSU#491 + BSU#492 to crm7's `scripts/` + `.github/workflows/` + `package.json`. Both repos' migrations land on the shared Supabase project so the gate must enforce on both code paths. Stacked: crm7#864 must merge first (adds the script + workflow), then crm7#868 (adds the tests + wires them into the workflow as a pre-lint step). Drains handoff bsuite#1267 (crm7 lint mirror) + bsuite#1276 (crm7 test mirror — this rotation's verification work).
- ⚠️ **45 remaining authenticated-callable `SECURITY DEFINER` functions** scoped for next DB rotation per handoff bsuite#1261 — per-function classification required (app-callable RPC vs RLS helper vs admin-only). The diff-only forward gate keeps NEW drift from landing while the historical audit proceeds.
- ⚠️ **`report_templates` permissive-policy consolidation** (4× WARN `multiple_permissive_policies`) scoped for a dedicated rotation per handoff bsuite#1261 — output-equivalence proof required across 9 overlapping policies (estimated 3-4h with test cases).

**SECURITY HARDENING — branding-sanitiser SEC-002/003 + cross-app CSP + dep CVEs**

- ✅ **BSU#485 branding sanitiser SEC-002/SEC-003 closed** — `src/lib/branding-sanitize.ts` + wiring into `applyBrandingVars` + `BrandingProvider.applyBrandingToRoot`. Three branding tables (`platform_branding`, `tenant_branding`, `tenant_app_branding`) are writable by semi-trusted admins, so a crafted `font_family` or `logo_url` value could break out of the CSS `url()` or font declaration and inject arbitrary CSS into every consumer app. Image URLs are scheme-allowlisted (`http(s)` + root-relative paths only); font values are rejected when they carry CSS breakout tokens (`;`, `{`, `}`, `<`, `>`, backslash); platform_name is escaped into its CSS string token. Pure, regex-free sanitiser.
- ✅ **crm7#857 mirror landed** — `src/lib/branding-sanitize.ts` + same wiring into `useBranding.applyBrandingVars` + `BrandingProvider`. crm7 is the highest-traffic D2C surface so this prioritisation is correct. Same vulnerable applyBrandingVars block was duplicated verbatim across all 5 D2C apps (BSU + crm7 + R80.3 + throughput + conduit); follow-up handoffs filed for R80.3 / throughput / conduit / braden mirrors per the same sanitiser pattern.
- ✅ **BSU#488 CSP hygiene** — removed dead `api.x.ai` allowance (no runtime references; AI traffic routes through `ai-gateway.vercel.sh` which is already allowlisted); added `pusher.com` for Vercel Live preview-deployment comment toolbar (Vercel Live upgrades to a WebSocket connection on Pusher under the hood; without it every preview/dev page load logs a CSP violation). Companion fixes: crm7 (#849 earlier); conduit#281 (Next.js `next.config.ts` form — namespace leak was 6-of-6 not 5-of-6); R80.3#276; throughput#190; braden CSP already had Pusher allowance.
- ✅ **DEPS CVE patches via pnpm.overrides** (Doctrine §5.1 patch/minor only):
  - **BSU#487 + throughput#187** → `ws@^8.20.1` patches GHSA-58qx-3vcg-4xpx (Uninitialized memory disclosure, current ws@8.20.0 affected). BSU: ws pulled in via puppeteer devDependency (production exposure nil, hygiene fix). Throughput: ws pulled in via `@langchain/community` + `langchain` runtime stack (production exposure). `pnpm audit --audit-level=moderate` drops 1→0 (BSU) and 2→1 (throughput; remaining uuid moderate from `@langchain/langgraph` requires major bump 10.x→11+ and is queued separately).
  - **crm7#854** → `qs@^6.15.2` patches GHSA-q8mj-m7cp-5q26 (qs DoS on null/undefined entries in comma-format arrays when `encodeValuesOnly` is set). qs is loaded in production via `googleapis-common` → `googleapis@171.4.0` runtime stack. Same patch/minor only override pattern. `pnpm audit --audit-level=moderate` drops 1→0.
- ✅ **crm7#850 edge fn hardening** — `report-delivery` + `tenant-management` edge functions: replaced hardcoded single-origin CORS with `_shared/cors.ts` per-request origin echo + allowlist; sanitised raw-error responses (no more stack traces leaking to the client); 8-case regression test on the CORS module covering production BSuite origins, localhost/127.0.0.1 dev, Vercel previews, `d.*.crm7.app` aliases, hostile-origin rejection (including `"*"` / `"null"` / look-alike-TLD vectors), and rejected-origin fallback to `ALLOWED_ORIGINS[0]` (NOT wildcard).

**A11Y — WCAG 2.1 AA cross-app wave**

- ✅ **BSU#489 WCAG 2.1 AA fixes on W0 uplift primitives** — 5 ARIA-only fixes on `DataTable` + `Picker` that cascade to every consumer (Reports, Pay Item Groups, Page Builder, Tenant Admin, Permission Matrix, all future W2/W4/W6 surfaces). Zero behaviour change for sighted users; strict additions to the accessibility tree. DataTable: `scope="col"` on header cells (SC 1.3.1/H63) for screen-reader column-cell association; `aria-sort` cycling on sortable column headers (SC 4.1.2 + WAI-ARIA 1.2 §6.6.7) so AT users get a signal that columns are sortable and current sort direction. Picker: parallel additions.
- ✅ **crm7#846 skip-link functional + icon-only labels** — SC 2.4.1: `SkipNavigation` linked to `#main-content` but the target `<div>` was not focusable, so activating the skip link scrolled the page without moving keyboard focus → next Tab returned to navigation (skip link silently broken). Converted both `#main-content` targets (public + authenticated shells) to `<main id="main-content" tabIndex={-1}>`, supplying the missing primary landmark for authenticated content. Dropped the now-nested `role="main"` from the 404 route and converted the prod-misconfig `<main>` in `protected-route.tsx` to a non-landmark alert region. Added accessible names to 4 icon-only buttons that rendered a bare `<X>`: EmailComposeDialog + EmailTemplateSelector close buttons, custom-field option-remove, avetmiss drawer close. Extended `wcag-static.test.ts` with a skip-link/main-landmark gate.
- ✅ **crm7#859 distinct dialog names + live regions + table caption** — SC 4.1.2: the CommandDialog shim auto-embeds a sr-only `DialogTitle/Description` so every dialog passes the bare 4.1.2 minimum, but all three live `CommandDialog` instances shipped with the same generic `"Command palette"` name → SR users couldn't distinguish them. Extended the shim with optional `title`/`description` props (current strings as defaults — back-compat) and passed distinctive names from the three consumers. Plus: live regions on Reports surface async state changes; table `<caption>` element on the Reports list.
- ✅ **crm7#860 `role="alert"` on four error-state surfaces** — SC 4.1.3: four user-facing surfaces (`/deals`, `/leads/edit`, two others) previously rendered "Failed to load …" fallbacks without programmatic announcement → SR users got no notification when a query failed. Surgical addition of `role="alert"` (with `aria-live="assertive"` on the two top-of-page error blocks where the error UI fully replaces the form/list).

**FEATURE — W2 Reports Task 6 first RPC + Pass 2 primitive completion**

- ✅ **crm7#855 `report_timesheet_summary` RPC landed** — the only one of 7 W2 Task 6 RPCs that fits the current production schema cleanly (after a small `client_name` ↔ `host_employer.name` vocabulary adaptation documented in the migration header). `SECURITY INVOKER` + locked `search_path` so existing RLS on `timesheets` / `people` / `host_employers` narrows the result set to the caller's visibility window (tenant, own apprentice, host_employer engagement, gto_staff scope) — no privilege escalation. Quoted-identifier parameter names match the seed's paramSchema keys exactly (`startDate` / `endDate` / `employeeIds` / `clientIds`) so the runner's param spread + `supabase.rpc` dispatch hits the right function. `total_count` carrier emitted on every row (constant per call) so the runner can extract `rows[0].total_count` before stripping. Page size clamped to 1000 (matches `MAX_PAGE_SIZE` in the runner). Backs the timesheet-summary platform-scope template seeded in `20260522020000_seed_codehouse_parity_report_templates.sql` — Codehouse Workforce One parity row 86 #1.
- ⚠️ **Other 6 W2 Task 6 RPCs scoped in handoff bsuite#1237** — each has a harder schema gap than the timesheet-summary vocabulary mismatch and needs an operator design call before implementation can proceed. Gap matrix in the handoff body:
  - `report_rejected_timesheets`: `timesheet_state` ENUM has no `'rejected'`/`'disputed'` values — needs ENUM extension + `rejected_at`/`rejected_by`/`rejection_reason` columns; operator must decide between state-machine extension vs. separate `timesheet_status` carrier.
  - `report_pay_item_group_hours`: depends on `pay_item_groups` + `timesheets.entries[].pay_item_id` linkage which may or may not be in prod.
  - `report_hours_by_work_type`: `timesheets.work_type` column doesn't exist yet (seed migration's own comment confirms); operator must decide between per-timesheet vs. per-entry granularity.
  - `report_consultant_kpi`: role-gating on `org_members.gto_role = 'field_officer'`; "active placements" definition; "missing timesheets" expected-vs-actual logic; revenue dependency on charge_rates/pay_items.
  - `report_coinvest_lsl`: requires `coinvest_lsl_accruals` table (Victorian construction LSL fund); operator must decide between dedicated table vs. computed from `timesheets.ordinary_hours` × accrual rate.
  - `report_pay_items_by_employee`: requires `pay_items` registry + `pay_periods` table presence.
- ✅ **crm7#853 FilterBar Pass 2 (canonical Doctrine §2.9 impl)** — promoted W0 Pass 1 `<PendingPrimitive>` stub to canonical implementation. Unblocks W2 Task 4 consumer slice (`/reports list uplift — DataTable grouped by scope + FilterBar`) plus every admin-list surface needing filtering (placements list, tenant list, audit log, candidate search). Features: free-text search with `Search` icon prefix (controlled — consumer debounces if server-side filtering); date range with side-by-side From/To inputs + per-range clear; active filter badges with per-badge X (calls `onRemoveChip`); `Add filter` button gated on `onAddChip` (opaque consumer-controlled chip-add UI — typically a Picker/CommandPalette popover); presets dropdown menu (each preset stores its own apply() callback — primitive does not own preset persistence); `Save as preset` button visible only when ≥1 active filter.

**AUTH stability — d.* preview-alias launcher fix + companion server-side gap**

- ✅ **BSU#494 app launcher d.* prefix preservation** — when BSU is served from `d.suite.crm7.app` (the stable dev-branch alias), the app launcher's tiles were sending the operator to production URLs (`crm.crm7.app`, `r8.crm7.app`, etc.) instead of d.* siblings. Root cause: `appList.ts` resolves cross-app URLs from `VITE_<APP>_URL` env vars; `vercel.json` hardcodes those to production URLs (correct for prod) but Vercel ships the same env vars to the development deployment because they're project-level. Fix: detect `window.location.hostname.startsWith('d.')` at runtime and rewrite each cross-app URL's hostname to its d.* sibling. Pure runtime transform — no env-var or vercel.json change, no production behaviour change (rewrite is a no-op when host doesn't start with `d.`). Submodule pointer bumped via bsuite#1278.
- ⚠️ **bsuite#1277 companion server-side gap filed** — when the operator initiates BS OAuth sign-in from a dev-branch alias (`d.crm.crm7.app`), the post-consent redirect lands them on `suite.crm7.app` instead of returning to the originating dev alias. Likely root cause: BS OAuth client `redirect_uris` allowlist in Supabase only has `https://*.crm7.app/auth/callback` entries — missing `https://d.*.crm7.app/auth/callback` variants for CRM7 / R80.3 / Conduit / Throughput / Braden. **Operator action required** (Supabase Studio access — not actionable from claude-loop): add the 5 `d.*` callback variants to the Supabase project `tuybltdrdefjblnplpqo` Authentication → URL Configuration → Additional Redirect URLs.

**Suite-wide hygiene — Node 24.x pin + miscellanea**

- ✅ **Node-24.x pin fix across all 7 repos** (BSU#490 / crm7#861 / conduit#282 / braden#298 / R80.3#277 / throughput#191) — `.node-version` content of `24` (bare major) was being interpreted as exactly `24.0.0` by actions/setup-node@v4, Vercel Deployment Check sandboxes, and most fnm/nvm wrappers. That exact version doesn't exist as a stable release, so the tool chain silently fell back to LTS (Node 22.22.2). Symptoms: every CI run logged `WARN Unsupported engine: wanted: {"node":"24"} (current: {"node":"v22.22.2"})`; Vercel Typecheck Deployment Check exit-137 OOM under Node 22's tighter sandbox memory limit; intermittent build failures correlated with sandbox memory pressure. Fix: industry-standard `24.x` form (any Node 24 release). Mirrors the workaround BSU's `e2e.yml` workflow already used explicitly. Both `.node-version` and `package.json engines.node` updated for consistency across the resolution chain. crm7#861 also bumped `NODE_OPTIONS` heap from 6144→8192 (1484 TS files exceeded the prior ceiling on Vercel Typecheck preview).
- ✅ **crm7#856 EmptyState uplift** — Doctrine §3.2 "Empty states are inviting, not punishing." Two surfaces (`EntityRelationshipTabs` — high leverage, used on every entity detail page; `DocumentsHub` tabs) were rendering bare grey-text empty branches with no path forward. Replaced both with the canonical W0 `EmptyState` primitive (illustration + headline + supporting text + CTA). Extended `RelationshipConfig` with optional `emptyState` block (`headline`/`subhead`/`primaryAction`) — strict superset of existing `emptyMessage` (back-compat). Cascades to 6+ consumers (apprentice / host / placement / qualification / timesheet / training_plan).
- ✅ **braden#297 AEO discoverability — llms.txt + llms-full.txt** — emerging `llmstxt.org` standard so AI search engines (Perplexity, ChatGPT Search, Claude, Google AI Overviews, Gemini) can ingest a curated, machine-friendly summary of `braden.com.au` without parsing the full SPA bundle. Closes the AEO (Answer Engine Optimization) discoverability gap called out in HubSpot's Spring 2026 release. Aligns Braden's corporate marketing surface with the 2026 AI-search default behaviour: agents check `/llms.txt` before crawling DOM. Two static-asset files added; no code, schema, or auth surface touched.
- ✅ **throughput#189 dead-code delete** — `src/lib/billing.ts` shipped 453 LOC of dead code (`BillingService`, `UsageTracker`, `SubscriptionPlan`, `SUBSCRIPTION_PLANS` all `export`ed but zero importers verified via cross-repo grep). File also harboured the only TYPES violations in throughput source: `private static stripe: any`; sole `@ts-expect-error` directive (factually wrong — `index.html` contains no Stripe script tag, so `Stripe(key)` would have thrown `ReferenceError` at runtime); `getBillingHistory(): Promise<any[]>`. Net deletion.
- ✅ **throughput#188 Skeleton sweep** — replaced 5 centered-spinner loading blocks across dashboard surfaces with shadcn-canonical content-shape `<Skeleton>` placeholders. 2026 SaaS dashboard doctrine (skeleton beats spinner for content surfaces — better perceived performance, less layout shift, lower CLS). New `src/components/ui/Skeleton.tsx` primitive (animate-pulse + rounded-md + bg-muted with `role="status"` / `aria-live="polite"` / `aria-label="Loading"`). Inline action spinners stay (`Saving...`, `Exporting...`, `Sending...` — correct usage for short-duration action feedback, not loading content).
- ✅ **throughput#185 THEME-REVIEW comment leak fix** (informational — landed 2026-05-21 cycle but worth noting for the docs trail) — an automated theme-annotation pass committed 103 `/* THEME-REVIEW: ... */` block comments across 47 files; ~31 sat in JSX children position so they rendered as literal visible text in the UI (the mobile bottom-nav floating action button, Supabase setup loading/error cards, etc.). All removed.
- ✅ **bsuite#1271 ship-cycle 2026-05-25 submodule pointer bump** — 6 submodule pointers refreshed across the parent monorepo (informational sync record).
- ✅ **bsuite#1272 BSuite Plan Dashboard refresh** — 22-PR ship-cycle landings ingested into the live dashboard at `https://garyocean428.github.io/bsuite/`.

**Open carry-forwards (re-listed at this rotation)**

| Ref | Owner | Status |
|---|---|---|
| bsuite#1209 — `isPkceError` regex audit across 4 sibling apps | next TYPES / TESTS rotation | P2 carry from 2026-05-20 DOCS |
| bsuite#1216 — W2 Reports Task 6/7/9 implementation | next FEATURE rotation | P2 — drained Tasks 1/1.5/2/3b/5/8 + Task 6 1-of-7; remaining 6 RPCs need operator schema decisions per bsuite#1237 |
| bsuite#1221 — schema-reconciliation regression sweep audit | next DB rotation | P2 — scripted audit of 270+ reconciled migrations vs. prod schema for silently-dropped objects (`template_key`-class regressions) |
| bsuite#1237 — W2 RPC schema-gap audit (6 remaining templates) | operator decision + next FEATURE rotation | P2 — 6 templates blocked on schema-design questions in handoff body |
| bsuite#1261 — 45× authenticated-callable SECURITY DEFINER function classification + `report_templates` permissive-policy consolidation | next DB rotation (or DB+2) | P2 — per-function audit needed; consolidation needs output-equivalence proof |
| bsuite#1267 — REVOKE-FROM-anon lint crm7 mirror | ship-all-apps merge | P2 — crm7#864 open (+ stacked crm7#868 tests) |
| bsuite#1276 — REVOKE-FROM-anon test suite crm7 mirror | ship-all-apps merge | P2 — crm7#868 open (drained this rotation) |
| bsuite#1277 — BS OAuth `redirect_uris` allowlist d.* variants | **operator** (Supabase Studio access) | P1 — blocks reliable d.* preview-alias OAuth login |
| Branding-sanitiser mirrors to R80.3 / throughput / conduit / braden | next SECURITY / FEATURE rotation | P2 — pattern landed in BSU#485 + crm7#857 |
| Slices 4–6 of bsuite#1015 §9.3 #1 PERF entrypoint lazy-shell | next PERF rotation | P3 — braden + throughput Vite `main.tsx` audit + conduit Next.js 16 `dynamic()` audit |
| 8 carry-forwards from prior 2026-05-23 evening section | various rotations | All still active — see 2026-05-23 evening section below |

## Recently Completed (as of 2026-05-23 — claude-loop ROADMAP rotation, evening: handoff drain + schema-reconciliation regression discovered + DataTable Pass 2 wave-override)

> Captures work landed in the 2026-05-23 09:00 → 09:15Z window (morning's DOCS rotation already shipped v5.13W earlier). Three substantive items: (1) **bsuite#1213 PERF rotation** — R80.3 entrypoint lazy-shell saved 176 KB from the main chunk; (2) **bsuite#1217 / crm7#852 DataTable Pass 2** — W2-prereq wave-override that replaced the Pass 1 PendingPrimitive stub with the canonical TanStack v8 implementation; (3) **bsuite#1216 handoff drain** — applied W2 Reports CRM7 Task 1.5 + Task 5 migrations to Supabase prod via Supabase MCP, *with an additional `template_key` column-restore pre-migration* that surfaced a real production regression — the `reconciled-2026-05-19` schema squash had silently dropped a column added by `20260508082500_host_monthly_pack_report` (GTO Catalogue G4), leaving the host-pack flow + the report-delivery edge function quietly broken since 2026-05-19. Restored via `restore_report_templates_template_key_after_reconciliation`; audit follow-up filed as bsuite#1221.

**PERF — R80.3 entrypoint lazy-shell (slice 3/4 of bsuite#1015 §9.3 #1)**

- ✅ **R80.3 `src/main.tsx` lazy-loaded 4 non-dominant routes — main chunk −176.70 kB (−34.4%)** ([R80.3#275](https://github.com/GaryOcean428/R80.3/pull/275), tracker [bsuite#1213](https://github.com/GaryOcean428/bsuite/issues/1213)) — pre-fix `src/main.tsx` synchronously imported FIVE route-level components (App, MarketingHome, AuthCallback, AuthLogin, ResetPassword), but path-based routing guarantees exactly ONE renders per request — the four non-dominant routes were dead weight in every authenticated user's initial bundle. Refactor: `React.lazy()` + `<Suspense>` for the 4 non-dominant routes (App stays eager — it's the LCP target for the steady-state authenticated user landing on `/calculator`). Main entry chunk `index-T-m0h4l3.js` 513.97 kB → `index-VIh6hqnj.js` 337.27 kB (`−176.70 kB`); headroom against 540 kB ceiling jumped from ~26 kB to ~203 kB. For the dominant case (authenticated user lands on `/calculator`) the saved JS is NOT parsed, NOT executed, NOT cached. The 138 KB MarketingHome chunk surprise (`magicui/magic-card` + `react-helmet-async` + marketing-page content) was the biggest single win — authenticated users were paying the marketing-landing JS cost on every load. **Pattern shape for slices 4-6:** audit `main.tsx` (or `_app.tsx` / `layout.tsx`) BEFORE auditing in-app routing — entrypoint dead-weight is invisible to App-level audits. Slices 4-6 (braden + throughput Vite + main.tsx audit, conduit Next.js 16 `dynamic()`) remain open as carry-forward.

**WAVE OVERRIDE — W2-prereq DataTable Pass 2 (canonical TanStack v8 impl)**

- ✅ **crm7 `src/components/uplift/DataTable.tsx` promoted stub → canonical Pass 2** ([crm7#852](https://github.com/GaryOcean428/crm7/pull/852), tracker [bsuite#1217](https://github.com/GaryOcean428/bsuite/issues/1217)) — W2 Reports Task 4 (`/reports list uplift — DataTable grouped by scope + FilterBar`) scoping discovered that the W0 DataTable + FilterBar primitives were still Pass 1 `<PendingPrimitive>` stubs rendering a yellow banner — not the canonical TanStack implementations per Doctrine §2.10. Surfaces that imported `DataTable` from the uplift barrel would have rendered the yellow stub banner instead of a real table (silent UX regression risk). Promoted DataTable to Pass 2 in a single PR with no consumers touched (interface is a strict superset of the W0 stub — future consumers can adopt without breaking changes). **Features shipped:** TanStack Table v8 + shadcn primitives; sortable headers with `aria-sort` cycling (ASC-first via `sortDescFirst: false` to match user intuition); column-visibility menu via shadcn `<DropdownMenuCheckboxItem>`; per-row actions menu (destructive styling + per-row disabled predicate); bulk selection with select-all/indeterminate checkbox + contextual action bar (only shown when ≥1 row selected); pagination footer with `aria-live="polite"`; CSV export via `@bsuite/data-export` (honours visible columns + `exportValue` override); print export via `window.print()`; default `uplift/EmptyState` when `data === []`; right-align column support. **Unblocks:** W2 Task 4 consumer slice (now pending the FilterBar Pass 2 promotion + the bsuite#1216 prod-migration drain); W4 Permissions Editor list; W5 Tenant Admin list; the audit log surface. 19-case unit test suite added.

**HANDOFF DRAIN — bsuite#1216 W2 Reports Task 1.5 + Task 5 migrations applied to Supabase prod**

- ✅ **3 migrations applied to Supabase project `tuybltdrdefjblnplpqo` via `mcp__supabase__apply_migration`** (claude-loop, this rotation):
  1. **`report_templates_scope_hierarchy`** (W2 Task 1.5) — 4-scope ENUM (`platform`/`enterprise`/`tenant`/`user`); `scope NOT NULL` + `user_id UUID` + `is_default BOOLEAN` columns; backfill from legacy `is_system`; `chk_report_templates_scope` 4-way CHECK constraint; `is_enterprise_admin()` + `auth_parent_tenant_id()` SECURITY DEFINER helpers (closing a real RLS-recursion footgun for enterprise sub-org visibility); 5 scope-aware RLS policies + service_role bypass; 2 partial indexes.
  2. **`restore_report_templates_template_key_after_reconciliation`** (NEW — pre-migration, real production bug fix) — `template_key TEXT` column + partial unique index + 8-row backfill. Discovered during the seed-apply attempt: the column was added by `20260508082500_host_monthly_pack_report` but DROPPED during the `reconciled-2026-05-19` schema squash. Stub didn't preserve the `ADD COLUMN`. App code in `src/pages/portal/host-reports.tsx` + the `report-delivery` edge function still queries `template_key` → silent production breakage since 2026-05-19 (5 days, no customer complaint because the host-pack flow is a monthly scheduled cron). Restored with idempotent `ADD COLUMN IF NOT EXISTS` + 8 GTO catalogue keys backfilled.
  3. **`seed_codehouse_parity_report_templates`** (W2 Task 5, rewritten) — 7 platform-scope templates (`timesheet-summary`, `pay-item-group-hours`, `hours-by-work-type`, `rejected-timesheets`, `consultant-kpi`, `coinvest-lsl`, `pay-items-by-employee`). Authoring deviation: upstream seed uses `ON CONFLICT (template_key) DO NOTHING` which fails on prod with `42P10: no unique or exclusion constraint matching the ON CONFLICT specification` because `template_key`'s unique index is partial (`WHERE template_key IS NOT NULL`) and PG requires the index predicate for inference. Rewrote each INSERT as `INSERT ... SELECT ... WHERE NOT EXISTS (SELECT 1 ... WHERE template_key = '<key>')` — same idempotent semantics. **Upstream patch needed** (filed as a non-blocking follow-up): `crm7/supabase/migrations/20260522020000_seed_codehouse_parity_report_templates.sql` will fail any future fresh-apply against a reconciled prod; patch the seed file in-repo to use the NOT EXISTS pattern.
- ✅ **Post-apply verification:** 4 new columns present; 7 platform seeds present; 6 RLS policies present (visibility + 4 scope writes + service_role); 2 helper fns present; check constraint present. Closes the handoff issue. **Unblocks** W2 Task 4 consumer slice + Tasks 6/7/9 + any future surface reading `scope` / `user_id` / `is_default` / `template_key` from `report_templates`.

**FOLLOW-UP FILED — schema-reconciliation regression sweep ([bsuite#1221](https://github.com/GaryOcean428/bsuite/issues/1221), P2)**

The `template_key` discovery is almost certainly NOT unique. The `reconciled-2026-05-19` squash collapsed ~270+ migrations into stubs; ANY `ALTER TABLE ... ADD COLUMN`, `CREATE TABLE`, `CREATE FUNCTION`, `CREATE POLICY`, or `CREATE INDEX` between 2026-05-08 and 2026-05-19 may have been silently dropped. Bsuite#1221 scopes a scripted audit (list reconciled migrations → fetch each original from `crm7@development` → diff CREATE statements → check existence in prod → file restore PRs per missing object). Picked up on the next DB rotation.
...
- **2026-05-23 v5.14W** (evening) — claude-loop ROADMAP rotation ([bsuite#1219](https://github.com/GaryOcean428/bsuite/issues/1219)):
  - **Rotation deliverable shipped this run:** drained handoff [bsuite#1216](https://github.com/GaryOcean428/bsuite/issues/1216) (W2 Reports CRM7 Task 1.5 + Task 5 migrations to Supabase prod, project `tuybltdrdefjblnplpqo`) via `mcp__supabase__apply_migration`. Required an additional `template_key` column-restore pre-migration that surfaced a real production regression — the `reconciled-2026-05-19` schema squash had silently dropped a column added by `20260508082500_host_monthly_pack_report`, leaving the host-pack flow (GTO Catalogue G4) + the `report-delivery` edge function quietly broken since 2026-05-19. Restored via `restore_report_templates_template_key_after_reconciliation` (idempotent, backfilled 8 GTO catalogue keys). Authored the W2 seed migration as `INSERT ... WHERE NOT EXISTS` instead of `ON CONFLICT (template_key) DO NOTHING` because PG cannot infer the partial unique index (`WHERE template_key IS NOT NULL`) without an explicit predicate clause — same idempotent semantics, no behaviour change. **Upstream patch follow-up needed** for `crm7/supabase/migrations/20260522020000_seed_codehouse_parity_report_templates.sql` so a future fresh-apply against a reconciled prod won't fail; noted in this section but not actioned this rotation to respect the migrations-immutable convention (filed as the next DB rotation candidate).
  - **Schema-reconciliation audit follow-up filed** as [bsuite#1221](https://github.com/GaryOcean428/bsuite/issues/1221) (P2, db-audit + security labels). The `template_key` discovery is unlikely to be unique — the reconciled-2026-05-19 squash collapsed ~270+ migrations into stubs; ANY `ALTER TABLE ... ADD COLUMN`, `CREATE TABLE`, `CREATE FUNCTION`, `CREATE POLICY`, or `CREATE INDEX` between 2026-05-08 and 2026-05-19 may have been silently dropped. Scoped a scripted approach (list reconciled migrations → fetch each original from `crm7@development` → diff CREATE statements → check existence in prod → file restore PRs per missing object). Picked up next DB rotation.
  - Added new "Recently Completed (as of 2026-05-23 evening)" section above the existing 2026-05-23 DOCS-rotation block. Captures: (a) the handoff drain + schema-reconciliation regression as the headline finding; (b) [bsuite#1213](https://github.com/GaryOcean428/bsuite/issues/1213) PERF — R80.3 `src/main.tsx` entrypoint lazy-shell shipped earlier this morning (−176.70 kB main chunk, −34.4%); (c) [bsuite#1217](https://github.com/GaryOcean428/bsuite/issues/1217) / [crm7#852](https://github.com/GaryOcean428/crm7/pull/852) — DataTable Pass 2 (canonical TanStack v8 impl) W2-prereq wave-override shipped earlier this morning; (d) the new bsuite#1221 audit follow-up.
  - Last Updated line bumped to reflect the handoff-drain + schema-regression as the rotation deliverable.
  - **§1.2 research_evidence (primary-source):** crm7 source migration files `supabase/migrations/20260522010000_report_templates_scope_hierarchy.sql` (blob `aa87514925bf0e38f4e572ef2777e65332b970e0`) + `20260522020000_seed_codehouse_parity_report_templates.sql` (blob `6472f177c1e55749eb421062d70baf9a324f4ee9`) fetched via `mcp__github__get_file_contents` on `GaryOcean428/crm7@development`; pre-existing `20260508082500_host_monthly_pack_report.sql` (blob `b0f1961240c4f1b21f51bc416116dc5bc0faf2ee`) — source of the lost `template_key` column; `mcp__github__search_code` to confirm `template_key` references in `src/pages/portal/host-reports.tsx` + `supabase/functions/report-delivery/index.ts` (proving the silent breakage); Supabase MCP `list_migrations` + `execute_sql` to verify pre-apply schema state; PG docs for ON CONFLICT partial-index inference requirement (https://www.postgresql.org/docs/current/sql-insert.html#SQL-ON-CONFLICT); AUTH_CANONICAL.md §RLS + Supabase RLS docs for the helper-function SECURITY DEFINER pattern. No blog posts; no third-party summaries.
  - **§9 Evidence:** §9.1 output-equivalence — migrations applied with `success: true` for all 3; post-apply verification query returned 4 columns / 7 platform seeds / 6 RLS policies / 2 helper fns / 1 check constraint (exact counts match the expected schema); §9.2 visual N/A (DB-only changes — no UI surface); self-report — the `template_key` reconciliation regression was unexpected mid-rotation and explicitly named in this entry; tests run — `mcp__Supabase__execute_sql` against `information_schema.columns`, `pg_policy`, `pg_proc`, `pg_constraint` post-apply; live verify — Supabase MCP attached to project `tuybltdrdefjblnplpqo` returns hardened state. Roadmap edit itself: file syntax preserved (markdown), version bumped 5.13W → 5.14W with matching Last Updated + revision-log entry.
