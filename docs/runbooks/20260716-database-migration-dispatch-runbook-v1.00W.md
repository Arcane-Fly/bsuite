...
**Related runbook:** [Parent Pointer Reconcile](20260716-parent-pointer-reconcile-runbook-v1.00W.md) — read that one first if you don't already understand submodule gitlinks
...
2. **Bump the parent submodule pointer.** The parent repo's `.gitmodules` gitlink for that submodule must be updated to the new commit SHA and pushed to the parent's `main`. See the [Parent Pointer Reconcile runbook](20260716-parent-pointer-reconcile-runbook-v1.00W.md) for the exact commands. **This step is not optional and not implied by step 1** — the parent repo has no automatic tracking of submodule commits.
3. **Dispatch the migration workflow:**
   ```bash
   gh workflow run supabase-migrate.yml --ref main -f submodule=<name>
   ```
   Valid `submodule` values: `all`, `root`, `crm7`, `R80.3`, `braden`, `business-suite-unified`, `conduit`, `throughput`, `schema-builder`. Use `--ref main` (the workflow only exists on `main`; dispatching against a feature branch will fail to find it or run stale workflow code).
...
- [ ] For scheduled/independent verification of the whole migration history (not just today's migration): `crm7/.github/workflows/prod-migration-history-audit.yml` runs daily and on dispatch — it flags rows in `schema_migrations` with `statement_count = 0` (recorded but never actually executed) as a class of drift distinct from the stale-pointer trap above

## Related

- [Parent Pointer Reconcile runbook](20260716-parent-pointer-reconcile-runbook-v1.00W.md) — the pointer-bump mechanics in detail
- [Edge Function Deploy runbook](20260716-edge-function-deploy-runbook-v1.00W.md) — the sibling pipeline for `supabase/functions/`, same stale-pointer risk applies
