...
A background process that also touches submodule working trees on every run is the hourly dashboard-refresh cron (`.github/workflows/refresh-dashboard-data.yml`, checkout with `submodules: recursive`) — but that workflow only ever commits `docs/dashboard/data/dashboard-data.json` and `docs/dashboard/index.html`; it does not commit gitlink changes, so it is not itself a source of pointer drift. Treat the "dirty submodule" state as a normal, load-bearing signal that a pointer bump is owed, not as evidence of a specific automated process forcing it.
