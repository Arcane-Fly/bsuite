...
**Access:** `platform_branding` itself carries an `INSERT`/`UPDATE` policy gated to `profiles.platform_role IN ('developer', 'platform_admin')` — anyone else attempting to write any row (including a per-app one) is rejected by RLS, not just by the UI. Reads are broader: `authenticated` sees every row; `anon` sees only the `'platform'` master row (through the `platform_branding_public` view, which additionally excludes `force_override_tenant_ids` from what any authenticated caller sees).

## Tenant switching — who can switch, and to what

Enforced inside the `log_tenant_switch(p_from_tenant_id, p_to_tenant_id, p_reason, p_ip_address, p_user_agent)` `SECURITY DEFINER` RPC (the authoritative check — the client's `checkSuperAdminStatus()` gates *visibility* of the switcher UI, but the RPC re-derives authorization independently):
