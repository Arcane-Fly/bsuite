...
- Canonical app URL env var map (`VITE_APP_URL`) for all 6 apps:
  ...
  - Braden: prod `https://www.braden.com.au`, dev `https://d.braden.com.au`
    ...
    - braden's `SEOHead` `BASE_URL` must be `BSUITE_PROD_URLS.braden` from `@bsuite/nav-core`, never env-derived: the preview bundle inlines `d.braden.com.au`, which would make the crawlable preview self-canonical. Braden development has this since `9dd2fdd`; braden main does not yet.
    - braden's `public/sitemap.xml`, `robots.txt`, `llms.txt` and `llms-full.txt` must use `www` URLs only, guarded by `braden/src/__tests__/canonical-host.test.ts`. Braden development has this since `9dd2fdd`; braden main does not yet.
