...
- The **core** — fetch, Supabase Realtime invalidation, loading / missing / error states, and the
  `silent` versus `message` miss contract — is app-agnostic and lives in the package. It is cored
  on crm7's implementation, which is the only one that draws real content.
...
- `@bsuite/page-builder` gains a `CustomPageRenderer` export and a minor version. Consumers adopt
  it on their own schedule, so ADR-0003's release-friction concern is bounded even though it no
  longer applies.
...
1. `CustomPageRenderer` lands in `@bsuite/page-builder`, cored on crm7's implementation, taking a
   widget-catalogue prop. Published as a minor version.
