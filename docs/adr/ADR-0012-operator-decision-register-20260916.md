...
| | |
|---|---|
| **Status** | Proposed — 12 open, 1 ruled and executed, 1 partially ruled |
| **Raised** | 2026-09-16 |
| **Decider** | Braden |
...
Thirteen decisions reserved to the operator, consolidated from one day's evidence rather than
sent piecemeal. Each carries **Situation, Evidence, Options, Precedent, Recommendation** — enough
...
| # | Decision | Status |
|---|---|---|
| 1 | Linear history on crm7 + business-suite-unified main | **RULED 2026-09-16 14:01 — "update linear history as needed i dont care about that one". EXECUTED:** `required_linear_history` removed from crm7 ruleset `12812477` and business-suite-unified ruleset `13700163`; both re-read live to confirm. Classic protection on all six repos already reported `false`. **Still open in this section:** `enforce_admins=true` on crm7 main and R80.4 main — not named in the ruling, and it changes who can bypass on a production trunk, so it is not assumed. |
| 2–14 | see below | Open |
