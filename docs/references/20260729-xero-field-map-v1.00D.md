...
| Pay Runs | https://developer.xero.com/documentation/api/payrollau/payruns | `PayrollCalendarID`, `PayRunID`, period dates, `PayRunStatus` (`DRAFT`\|`POSTED`), `PaymentDate`, `PayslipMessage`, `Payslips[]`, and org-level totals `Wages`/`Deductions`/`Tax`/`Super`/`Reimbursement`/`NetPay` | Aggregate financial totals per run — post-hoc reconciliation source, not a rate source. Note: **"Effective November 2025, you can only create and post a single pay run per request."** |
...
| Payroll Calendars | https://developer.xero.com/documentation/api/payrollau/payrollcalendars | `PayrollCalendarID`, `CalendarType` (`WEEKLY`\|`FORTNIGHTLY`\|`FOURWEEKLY`\|`MONTHLY`\|`TWICEMONTHLY`\|`QUARTERLY`), name, start date, payment date | Cadence metadata for linking pay-run scheduling |
| Timesheets | https://developer.xero.com/documentation/api/payrollau/timesheets | V2: `TimesheetID`, `PayrollCalendarID`, `EmployeeID`, `StartDate`/`EndDate`, `Status` (`Draft`\|`Requested`\|`Approved`\|`Declined`\|`Completed`), `TotalHours`; supports approve/revert-to-draft/delete/line-level PUT/DELETE; filterable by `employeeId`, `payrollCalendarId`, `status`, date range | Closest Xero surface to our own timesheet/Anytime-equivalent module — could be a sync target, not a rate source |
