...
| P0-2 | Delete `VITE_STRIPE_SECRET_KEY` from `business-suite` Vercel project | Operator | `vercel env ls business-suite` returns no row | `docs/20260424-env-var-audit-findings-v1.00A.md` §2.A |
| P0-3 | Delete `VITESUPABASE_ANON_KEY` typo from `throughput` Vercel project | Operator | `vercel env ls throughput` returns no typo row | `docs/20260424-env-var-audit-findings-v1.00A.md` §2.C |
| P0-4 | Add 7 `RAM_*` vars to `crm7` Vercel project (deploy-time bomb) | Operator | Call to `createRamClientFromEnv()` in a CRM7 production runtime does not throw | `docs/20260424-env-var-audit-findings-v1.00A.md` §3; `docs/20260424-env-var-contributing-rules-v1.00W.md` §5 |
| P0-5 | Fix `crm7/src/lib/pipelines/xeroInvoiceAdapter.ts:62` — stop reading `VITE_XERO_CLIENT_SECRET`; move adapter behind an edge fn reading `XERO_CLIENT_SECRET` via `Deno.env.get()` | CRM7 / Claude Code | Post-build grep `grep -rE '(sb_secret_\|sk_live_)' crm7/dist/` returns zero hits | `docs/20260424-env-var-audit-findings-v1.00A.md` §2.B |
...
| P0-14 | `git rm crm7/APPLY_THIS_SQL.sql`; BFG history rewrite if file contains credentials | CRM7 / Operator | File absent from `main` tip + history scrub verified | `docs/plans/20260423-bsuite-production-plan-v1.00W.md` §P0-J08 |
...
| P1-3 | CAW-V7a — remove `crm7/src/services/tenantService.ts:63` writes to `tenant_branding` / `platform_branding` (BSU-owned) | CRM7 / Claude Code | CRM7 consumes via `<BrandingProvider>` only; authoring moved to BSU `/admin/branding` | `docs/20260423-cross-app-write-audit-v1.00W.md` §V7 |
| P1-4 | CAW-V7b — remove CRM7 `src/pages/settings/schema-builder/*` INSERTs/UPSERTs to `custom_pages` / `custom_page_blocks`; consume via `<TenantLayoutSlot>` only | CRM7 / Claude Code | Only BSU `/developer/pages` authors; CRM7 schema-builder is read-only for those tables | `docs/20260423-cross-app-write-audit-v1.00W.md` §V7 |
...
| P1-7 | CAW-braden-debt-001 — remove or repoint `braden/src/lib/tasks/taskService.ts:37` `.from('users')` dead-code path | Braden / Claude Code | Function either returns real `auth.users` via RLS-safe view or is deleted | `docs/20260423-cross-app-write-audit-v1.00W.md` §V3 braden observation |
| P1-8 | CAW-throughput-debt-001 — annotate `throughput/supabase/migrations/20251014120000_unified_business_suite_schema.sql` as "REFERENCE ONLY — NEVER APPLY" or split to throughput-owned tables only | Throughput / Claude Code | Migration file header comment present or file split; CI asserts no dormant duplicate declarations | `docs/20260423-cross-app-write-audit-v1.00W.md` §V4 throughput observation |
...
| P1-69 | Throughput Tailwind v3 → v4; wire `@bsuite/theme/preset-v4.css`; bump `@supabase/supabase-js` `^2.39.7` → `^2.103.0` | Throughput / Cascade | Tailwind v4 config loads; tokens resolve | `docs/plans/20260423-bsuite-production-plan-v1.00W.md` §Phase 10 |
