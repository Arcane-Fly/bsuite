...
7. **Award-interpretation parity — ARCHITECTURAL (audited 2026-07-23):** @bsuite/charge-calc is a charge-RATE modelling engine (annual cost → hourly charge rate for GTO quotes), NOT a timesheet-interpretation engine. Only 1/9 AnyTime rule types at parity (Weekly Award Rules 🟡), 7 ❌ absent (Daily shift rules, Penalty MF+threshold, Max-hours-break, Daily Award, Fortnightly Award, Allowance Rules+MF+cap, auto-coding+override). **Parity requires a NEW timesheet-interpretation engine** — a distinct architectural layer (per-shift conditional rule matching), not a charge-calc extension. Candidate home: R80.3 payroll composer (#320/#321 chain) or a new @bsuite/award-interpreter package.
