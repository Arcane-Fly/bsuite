...
| **P0-10** | **Supabase Vault RPC migration completeness** — create `email_integration_set_encrypted_token`; migrate plaintext `access_token`/`refresh_token`/`smtp_password`/`imap_password` in `email_integrations`; finish Vault-backed secrets for edge functions still reading `pg_settings`. **Progress 2026-06-04:** `crm7#997` moved Google/Microsoft email OAuth callbacks off the legacy combined `encryption_key_id` Vault JSON secret and now stores access/refresh tokens through `email_integration_set_encrypted_token` per-token Vault columns with plaintext token columns nulled. | CRM7 edge fns / Claude Code | Remaining: live DB confirmation that existing plaintext/legacy rows are migrated and no other email edge functions read legacy `pg_settings`/combined-secret paths. | Finish-line roadmap P0-10; `crm7#997` |
| **P0-11** | **OAuth state HMAC signing** — sign OAuth state with HMAC-SHA256 + verify on callback in `oauth-google-email` + `oauth-microsoft-email` edge functions. **Progress 2026-06-04:** HMAC state helper remains wired in both callbacks and was revalidated with `_shared/__tests__/oauth-state.test.ts` in `crm7#997`. | CRM7 edge fns / Claude Code | Remaining: browser/edge CSRF replay coverage against the deployed callback path before closing the row. | Finish-line P0-11; `crm7#997` |
...
- 2026-05-12 v1.00W — added BL-013 (Stripe FDW doctrine): new Stripe data reads move to Supabase Wrappers Stripe FDW (`wrappers` + `stripe_fdw` + `stripe` foreign tables) with service-role-only SECURITY DEFINER RPC wrappers; Stripe webhooks and write flows remain edge functions.
